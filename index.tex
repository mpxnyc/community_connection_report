...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\#pat  \textless{}{-} tidyjson::read\_json("\_config.json")[["..JSON"]][[1]][["settings"]][["github{-}token"]] \# update packages}
\CommentTok{\#remotes::install\_github("KeletsoMakofane/mpxnyc", auth\_token = pat)}
\end{Highlighting}
\end{Shaded}
...
\hypertarget{related-research-sec-introduction}{%
\chapter{*Related research
\{\#sec-introduction\}}\label{related-research-sec-introduction}}
...
{\centering \includegraphics{1_introduction/1_3__intro_related_research_files/figure-pdf/unnamed-chunk-2-1.pdf}
...
{\centering \includegraphics{2_methods/../_const/images/image_app_graphic.png}
...
{\centering \includegraphics{2_methods/../_const/images/image_campaign_graphic.png}
...
{\centering \includegraphics{2_methods/2_2_methods_data_files/figure-pdf/unnamed-chunk-2-1.pdf}
...
{\centering \includegraphics{2_methods/2_3_methods_measures_files/figure-pdf/fig-example-bipartite-1.pdf}
...
district. This relation can be because of a residence or attended
gathering in that community district. Related to this network are the
community district network and participant network, described below.
...
{\centering \includegraphics{2_methods/2_3_methods_measures_files/figure-pdf/fig-example-community-1.pdf}
...
had a residence or gathering in both places. The weight is given by the
count of such individuals. Of interest is the influence each community
district has in city-wide outbreak dynamics. It is measured using
community district social and spatial reach.
...
{\centering \includegraphics{2_methods/2_3_methods_measures_files/figure-pdf/fig-example-participant-1.pdf}
...
gathering in a common community district. The edge weight is given by
the count of community districts the pair share. Of interest are the
...
\hypertarget{gathering-edges}{%
\subsection{Gathering edges}\label{gathering-edges}}

Participants who indicated recent physical or sexual contact at a
gathering were asked for the locations of each of gatherings where the
contact happened. They are each represented as an edge between the
participant and the community district containing the gathering.

\hypertarget{event-type}{%
\subsubsection{Event type}\label{event-type}}

Participants were asked the type of event each gathering was (dance
...
Participants were also asked if they had sexual contact at each
gathering.
...
and gathering by categorizing gatherings into those that are in the same
...
{\centering \includegraphics{2_methods/2_3_methods_measures_files/figure-pdf/unnamed-chunk-6-1.pdf}
...
levels of the race-gender variable. We also tabulated event type against
sexual contact, and against distance from home.

To understand how participants move across community districts from home
to gatherings, we cross-tabulated home community district (the community
district of the participants' residence) against gathering community
district and visualized the resulting table. To identify community
districts that tend to receive more participants from residences in
outside community districts, and those which tend to send participants,
we subtracted the proportion of residences in each community district
from the proportion of gatherings The result of this calculation is
...
district is to immunize each individual who had a gathering or residence
in that community district. We assume that once a person is immunized,
they are unable to acquire or transmit the pathogen, and we assume that
everyone can only be immunized once. We assessed two strategies for
prioritizing community districts: the contact-neutralizing strategy and
...
number of gatherings and residences they contain (counting only the
...
participants 7 times with replacement.
...
{\centering \includegraphics{2_methods/2_4_methods_statistical_analysis_files/figure-pdf/unnamed-chunk-3-1.pdf}
...
\FunctionTok{make\_plotdata\_racegender\_radar\_grid}\NormalTok{() }\SpecialCharTok{|\textgreater{}}
  ...
  \FunctionTok{scale\_radial\_mpxnyc}\NormalTok{() }\SpecialCharTok{+}
...
{\centering \includegraphics{3_results/3_1__results_participants_files/figure-pdf/fig-racegender-radar-grid-1.pdf}
...
\hypertarget{home-neighborhood}{%
\section{Home neighborhood}\label{home-neighborhood}}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{make\_tabledata\_residence }\OtherTok{\textless{}{-}} \ControlFlowTok{function}\NormalTok{()\{}
\NormalTok{  targets}\SpecialCharTok{::}\FunctionTok{tar\_read}\NormalTok{(data\_bipartite\_graph\_collected) }\SpecialCharTok{|\textgreater{}}
\NormalTok{    tidygraph}\SpecialCharTok{::}\FunctionTok{activate}\NormalTok{(nodes)}\SpecialCharTok{|\textgreater{}}
    \FunctionTok{data.frame}\NormalTok{() }\SpecialCharTok{|\textgreater{}}
\NormalTok{    dplyr}\SpecialCharTok{::}\FunctionTok{filter}\NormalTok{(type) }\SpecialCharTok{|\textgreater{}}
\NormalTok{    dplyr}\SpecialCharTok{::}\FunctionTok{group\_by}\NormalTok{(borough, community, neighborhood) }\SpecialCharTok{|\textgreater{}}
\NormalTok{    dplyr}\SpecialCharTok{::}\FunctionTok{summarize}\NormalTok{(}\AttributeTok{count =}\NormalTok{ dplyr}\SpecialCharTok{::}\FunctionTok{n}\NormalTok{()) }\SpecialCharTok{|\textgreater{}}
\NormalTok{    dplyr}\SpecialCharTok{::}\FunctionTok{arrange}\NormalTok{(}\SpecialCharTok{{-}}\NormalTok{count) }\SpecialCharTok{|\textgreater{}}
...
\NormalTok{scale\_labels }\OtherTok{\textless{}{-}} \FunctionTok{c}\NormalTok{(}\StringTok{"0"}\NormalTok{, }\StringTok{"1{-}10"}\NormalTok{, }\StringTok{"11{-}20"}\NormalTok{, }\StringTok{"21{-}30"}\NormalTok{, }\StringTok{"31+"}\NormalTok{)}

\NormalTok{mpxnyc}\SpecialCharTok{::}\NormalTok{neighborhood\_sf\_obj }\SpecialCharTok{|\textgreater{}}
\NormalTok{   sf}\SpecialCharTok{::}\FunctionTok{st\_as\_sf}\NormalTok{() }\SpecialCharTok{|\textgreater{}}
\NormalTok{  dplyr}\SpecialCharTok{::}\FunctionTok{left\_join}\NormalTok{(}\FunctionTok{make\_tabledata\_residence}\NormalTok{(), }\AttributeTok{by =} \StringTok{"neighborhood"}\NormalTok{) }\SpecialCharTok{|\textgreater{}}
\NormalTok{  dplyr}\SpecialCharTok{::}\FunctionTok{mutate}\NormalTok{(}\AttributeTok{count =} \FunctionTok{ifelse}\NormalTok{(}\FunctionTok{is.na}\NormalTok{(count), }\DecValTok{0}\NormalTok{, count)) }\SpecialCharTok{|\textgreater{}}
\NormalTok{  dplyr}\SpecialCharTok{::}\FunctionTok{mutate}\NormalTok{(}\AttributeTok{count\_cat =} \FunctionTok{cut}\NormalTok{(count, }\FunctionTok{c}\NormalTok{(}\SpecialCharTok{{-}}\DecValTok{1}\NormalTok{,}\DecValTok{0}\NormalTok{,}\DecValTok{10}\NormalTok{,}\DecValTok{20}\NormalTok{,}\DecValTok{30}\NormalTok{,}\DecValTok{76}\NormalTok{), scale\_labels)) }\SpecialCharTok{|\textgreater{}}
\NormalTok{  dplyr}\SpecialCharTok{::}\FunctionTok{arrange}\NormalTok{(}\SpecialCharTok{{-}}\NormalTok{count) }\SpecialCharTok{|\textgreater{}}
\NormalTok{  ggplot2}\SpecialCharTok{::}\FunctionTok{ggplot}\NormalTok{() }\SpecialCharTok{+}
\NormalTok{  ggplot2}\SpecialCharTok{::}\FunctionTok{geom\_sf}\NormalTok{(}\AttributeTok{data =}\NormalTok{ mpxnyc}\SpecialCharTok{::}\NormalTok{community\_sf\_obj, }\AttributeTok{fill =} \StringTok{"white"}\NormalTok{, }\AttributeTok{linewidth =} \DecValTok{0}\NormalTok{, }\AttributeTok{color =} \ConstantTok{NA}\NormalTok{) }\SpecialCharTok{+}
\NormalTok{  ggplot2}\SpecialCharTok{::}\FunctionTok{geom\_sf}\NormalTok{(}\AttributeTok{data =}\NormalTok{ mpxnyc}\SpecialCharTok{::}\NormalTok{community\_sf\_obj, }\AttributeTok{fill =} \StringTok{"\#009BE8"}\NormalTok{, }\AttributeTok{linewidth =} \DecValTok{0}\NormalTok{, }\AttributeTok{color =} \ConstantTok{NA}\NormalTok{, }\AttributeTok{alpha =} \FloatTok{0.1}\NormalTok{) }\SpecialCharTok{+}
\NormalTok{  ggplot2}\SpecialCharTok{::}\FunctionTok{geom\_sf}\NormalTok{(ggplot2}\SpecialCharTok{::}\FunctionTok{aes}\NormalTok{(}\AttributeTok{alpha =}\NormalTok{ count\_cat), }\AttributeTok{linewidth =} \DecValTok{0}\NormalTok{, }\AttributeTok{fill =} \StringTok{"\#009BE8"}\NormalTok{, }\AttributeTok{color =} \StringTok{"\#C5EFFF"}\NormalTok{) }\SpecialCharTok{+}
\NormalTok{  ggplot2}\SpecialCharTok{::}\FunctionTok{geom\_sf}\NormalTok{(}\AttributeTok{data =}\NormalTok{ mpxnyc}\SpecialCharTok{::}\NormalTok{community\_sf\_obj, }\AttributeTok{fill =} \ConstantTok{NA}\NormalTok{, }\AttributeTok{linewidth =} \FloatTok{0.3}\NormalTok{, }\AttributeTok{color =} \StringTok{"black"}\NormalTok{) }\SpecialCharTok{+}
\NormalTok{  ggplot2}\SpecialCharTok{::}\FunctionTok{scale\_alpha\_discrete}\NormalTok{(}\StringTok{"Participant}\SpecialCharTok{\textbackslash{}n}\StringTok{count"}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{theme\_mpxnyc\_blank}\NormalTok{(}
    \AttributeTok{plot.background =}\NormalTok{ ggplot2}\SpecialCharTok{::}\FunctionTok{element\_rect}\NormalTok{(}\AttributeTok{fill =} \StringTok{"white"}\NormalTok{),}
...
{\centering \includegraphics{3_results/3_1__results_participants_files/figure-pdf/unnamed-chunk-4-1.pdf}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{table\_data }\OtherTok{\textless{}{-}} \FunctionTok{make\_tabledata\_residence}\NormalTok{()}

\NormalTok{labelled}\SpecialCharTok{::}\FunctionTok{var\_label}\NormalTok{(table\_data}\SpecialCharTok{$}\NormalTok{neighborhood) }\OtherTok{\textless{}{-}} \StringTok{"Neighborhood"}
\NormalTok{labelled}\SpecialCharTok{::}\FunctionTok{var\_label}\NormalTok{(table\_data}\SpecialCharTok{$}\NormalTok{community) }\OtherTok{\textless{}{-}} \StringTok{"Community district"}
\NormalTok{labelled}\SpecialCharTok{::}\FunctionTok{var\_label}\NormalTok{(table\_data}\SpecialCharTok{$}\NormalTok{borough) }\OtherTok{\textless{}{-}} \StringTok{"Borough"}
\NormalTok{labelled}\SpecialCharTok{::}\FunctionTok{var\_label}\NormalTok{(table\_data}\SpecialCharTok{$}\NormalTok{count) }\OtherTok{\textless{}{-}} \StringTok{"Count of participants"}
\NormalTok{labelled}\SpecialCharTok{::}\FunctionTok{var\_label}\NormalTok{(table\_data}\SpecialCharTok{$}\NormalTok{proportion) }\OtherTok{\textless{}{-}} \StringTok{"Proportion of participants"}

\NormalTok{ table\_data }\SpecialCharTok{|\textgreater{}}
\NormalTok{  dplyr}\SpecialCharTok{::}\FunctionTok{ungroup}\NormalTok{() }\SpecialCharTok{|\textgreater{}}
\NormalTok{  dplyr}\SpecialCharTok{::}\FunctionTok{filter}\NormalTok{(count }\SpecialCharTok{\textgreater{}} \DecValTok{29}\NormalTok{) }\SpecialCharTok{|\textgreater{}}
\NormalTok{   dplyr}\SpecialCharTok{::}\FunctionTok{select}\NormalTok{(neighborhood, community, borough, count, proportion) }\SpecialCharTok{|\textgreater{}}
...
\NormalTok{     gt}\SpecialCharTok{::}\FunctionTok{tab\_header}\NormalTok{(}
    \AttributeTok{title =} \StringTok{"Most popular neighborhoods for primary residence"}\NormalTok{,}
    \AttributeTok{subtitle =} \StringTok{"MPX NYC 2022"}
\NormalTok{  ) }\SpecialCharTok{|\textgreater{}}
\NormalTok{   gt}\SpecialCharTok{::}\FunctionTok{fmt\_percent}\NormalTok{(proportion, }\AttributeTok{decimals =} \DecValTok{0}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{longtable*}{lllrr}
\caption*{
{\large Most popular neighborhoods for primary residence} \\ 
{\small MPX NYC 2022}
} \\ 
\toprule
Neighborhood & Community district & Borough & Count of participants & Proportion of participants \\ 
\midrule\addlinespace[2.5pt]
Hell's Kitchen & MN04 & Manhattan & 76 & $6\%$ \\ 
Bedford-Stuyvesant (West) & BK03 & Brooklyn & 50 & $4\%$ \\ 
Harlem (North) & MN10 & Manhattan & 49 & $4\%$ \\ 
Crown Heights (North) & BK08 & Brooklyn & 45 & $3\%$ \\ 
Hamilton Heights-Sugar Hill & MN09 & Manhattan & 39 & $3\%$ \\ 
Chelsea-Hudson Yards & MN04 & Manhattan & 38 & $3\%$ \\ 
Washington Heights (South) & MN12 & Manhattan & 36 & $3\%$ \\ 
Astoria (Central) & QN01 & Queens & 36 & $3\%$ \\ 
Bushwick (East) & BK04 & Brooklyn & 35 & $3\%$ \\ 
Upper West Side (Central) & MN07 & Manhattan & 30 & $2\%$ \\ 
\bottomrule
\end{longtable*}
...
{\centering \includegraphics{3_results/3_1__results_participants_files/figure-pdf/unnamed-chunk-10-1.pdf}
...
\chapter{Gatherings}\label{sec-venues}}

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{make\_plotdata\_placetype\_bar\_grid}\NormalTok{() }\SpecialCharTok{|\textgreater{}}
  \FunctionTok{plot\_bar\_stratified2}\NormalTok{() }\SpecialCharTok{+}
  \FunctionTok{scale\_axis\_mpxnyc}\NormalTok{() }\SpecialCharTok{+}
  \FunctionTok{scale\_fill\_mpxnyc}\NormalTok{(}\AttributeTok{name =} \StringTok{""}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{theme\_mpxnyc\_bar\_places}\NormalTok{() }
\end{Highlighting}
\end{Shaded}

\begin{figure}[H]

{\centering \includegraphics{3_results/3_2__results_contact_venues_files/figure-pdf/fig-placetype-bar-grid-1.pdf}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{make\_tabledata\_gatherings }\OtherTok{\textless{}{-}} \ControlFlowTok{function}\NormalTok{()\{}
\NormalTok{  targets}\SpecialCharTok{::}\FunctionTok{tar\_read}\NormalTok{(data\_bipartite\_graph\_collected) }\SpecialCharTok{|\textgreater{}}
\NormalTok{    tidygraph}\SpecialCharTok{::}\FunctionTok{activate}\NormalTok{(edges)}\SpecialCharTok{|\textgreater{}}
    \FunctionTok{data.frame}\NormalTok{() }\SpecialCharTok{|\textgreater{}}
\NormalTok{    dplyr}\SpecialCharTok{::}\FunctionTok{group\_by}\NormalTok{(place\_borough, place\_community, place\_neighborhood) }\SpecialCharTok{|\textgreater{}}
\NormalTok{    dplyr}\SpecialCharTok{::}\FunctionTok{summarize}\NormalTok{(}\AttributeTok{count =}\NormalTok{ dplyr}\SpecialCharTok{::}\FunctionTok{n}\NormalTok{()) }\SpecialCharTok{|\textgreater{}}
\NormalTok{    dplyr}\SpecialCharTok{::}\FunctionTok{rename}\NormalTok{(}\AttributeTok{borough =}\NormalTok{ place\_borough, }\AttributeTok{community =}\NormalTok{ place\_community, }\AttributeTok{neighborhood =}\NormalTok{ place\_neighborhood) }\SpecialCharTok{|\textgreater{}}
\NormalTok{    dplyr}\SpecialCharTok{::}\FunctionTok{arrange}\NormalTok{(}\SpecialCharTok{{-}}\NormalTok{count) }\SpecialCharTok{|\textgreater{}}
\NormalTok{    dplyr}\SpecialCharTok{::}\FunctionTok{ungroup}\NormalTok{() }\SpecialCharTok{|\textgreater{}}
\NormalTok{    dplyr}\SpecialCharTok{::}\FunctionTok{filter}\NormalTok{(}\SpecialCharTok{!}\FunctionTok{is.na}\NormalTok{(borough)) }\SpecialCharTok{|\textgreater{}}
\NormalTok{    dplyr}\SpecialCharTok{::}\FunctionTok{mutate}\NormalTok{(}\AttributeTok{proportion =}\NormalTok{ count }\SpecialCharTok{/} \FunctionTok{sum}\NormalTok{(count))}
\NormalTok{\}}
...
\NormalTok{mpxnyc}\SpecialCharTok{::}\NormalTok{neighborhood\_sf\_obj }\SpecialCharTok{|\textgreater{}}
\NormalTok{   sf}\SpecialCharTok{::}\FunctionTok{st\_as\_sf}\NormalTok{() }\SpecialCharTok{|\textgreater{}}
\NormalTok{  dplyr}\SpecialCharTok{::}\FunctionTok{left\_join}\NormalTok{(}\FunctionTok{make\_tabledata\_gatherings}\NormalTok{(), }\AttributeTok{by =} \StringTok{"neighborhood"}\NormalTok{) }\SpecialCharTok{|\textgreater{}}
\NormalTok{  dplyr}\SpecialCharTok{::}\FunctionTok{mutate}\NormalTok{(}\AttributeTok{count =} \FunctionTok{ifelse}\NormalTok{(}\FunctionTok{is.na}\NormalTok{(count), }\DecValTok{0}\NormalTok{, count)) }\SpecialCharTok{|\textgreater{}}
\NormalTok{  dplyr}\SpecialCharTok{::}\FunctionTok{mutate}\NormalTok{(}\AttributeTok{count\_cat =} \FunctionTok{cut}\NormalTok{(count, }\FunctionTok{c}\NormalTok{(}\SpecialCharTok{{-}}\DecValTok{1}\NormalTok{,}\DecValTok{0}\NormalTok{,}\DecValTok{10}\NormalTok{,}\DecValTok{20}\NormalTok{,}\DecValTok{30}\NormalTok{,}\DecValTok{100}\NormalTok{), scale\_labels)) }\SpecialCharTok{|\textgreater{}}
\NormalTok{  dplyr}\SpecialCharTok{::}\FunctionTok{arrange}\NormalTok{(}\SpecialCharTok{{-}}\NormalTok{count) }\SpecialCharTok{|\textgreater{}}
\NormalTok{  ggplot2}\SpecialCharTok{::}\FunctionTok{ggplot}\NormalTok{() }\SpecialCharTok{+}
\NormalTok{  ggplot2}\SpecialCharTok{::}\FunctionTok{geom\_sf}\NormalTok{(}\AttributeTok{data =}\NormalTok{ mpxnyc}\SpecialCharTok{::}\NormalTok{community\_sf\_obj, }\AttributeTok{fill =} \StringTok{"white"}\NormalTok{, }\AttributeTok{linewidth =} \DecValTok{0}\NormalTok{, }\AttributeTok{color =} \ConstantTok{NA}\NormalTok{) }\SpecialCharTok{+}
\NormalTok{  ggplot2}\SpecialCharTok{::}\FunctionTok{geom\_sf}\NormalTok{(}\AttributeTok{data =}\NormalTok{ mpxnyc}\SpecialCharTok{::}\NormalTok{community\_sf\_obj, }\AttributeTok{fill =} \StringTok{"\#F73C95"}\NormalTok{, }\AttributeTok{linewidth =} \DecValTok{0}\NormalTok{, }\AttributeTok{color =} \ConstantTok{NA}\NormalTok{, }\AttributeTok{alpha =} \FloatTok{0.1}\NormalTok{) }\SpecialCharTok{+}
\NormalTok{  ggplot2}\SpecialCharTok{::}\FunctionTok{geom\_sf}\NormalTok{(ggplot2}\SpecialCharTok{::}\FunctionTok{aes}\NormalTok{(}\AttributeTok{alpha =}\NormalTok{ count\_cat), }\AttributeTok{linewidth =} \DecValTok{0}\NormalTok{, }\AttributeTok{fill =} \StringTok{"\#F73C95"}\NormalTok{, }\AttributeTok{color =} \StringTok{"\#FF99C5"}\NormalTok{) }\SpecialCharTok{+}
\NormalTok{  ggplot2}\SpecialCharTok{::}\FunctionTok{geom\_sf}\NormalTok{(}\AttributeTok{data =}\NormalTok{ mpxnyc}\SpecialCharTok{::}\NormalTok{community\_sf\_obj, }\AttributeTok{fill =} \ConstantTok{NA}\NormalTok{, }\AttributeTok{linewidth =} \FloatTok{0.3}\NormalTok{, }\AttributeTok{color =} \StringTok{"black"}\NormalTok{) }\SpecialCharTok{+}
\NormalTok{  ggplot2}\SpecialCharTok{::}\FunctionTok{scale\_alpha\_discrete}\NormalTok{(}\StringTok{"Gatherings}\SpecialCharTok{\textbackslash{}n}\StringTok{count"}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{theme\_mpxnyc\_blank}\NormalTok{(}
    \AttributeTok{plot.background =}\NormalTok{ ggplot2}\SpecialCharTok{::}\FunctionTok{element\_rect}\NormalTok{(}\AttributeTok{fill =} \StringTok{"white"}\NormalTok{),}
    \AttributeTok{legend.position =} \StringTok{"right"}
\NormalTok{    )}
...
{\centering \includegraphics{3_results/3_2__results_contact_venues_files/figure-pdf/unnamed-chunk-4-1.pdf}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{table\_data }\OtherTok{\textless{}{-}} \FunctionTok{make\_tabledata\_gatherings}\NormalTok{() }\SpecialCharTok{|\textgreater{}}
\NormalTok{  dplyr}\SpecialCharTok{::}\FunctionTok{filter}\NormalTok{(count }\SpecialCharTok{\textgreater{}} \DecValTok{16}\NormalTok{) }\SpecialCharTok{|\textgreater{}}
\NormalTok{  dplyr}\SpecialCharTok{::}\FunctionTok{ungroup}\NormalTok{() }\SpecialCharTok{|\textgreater{}}
\NormalTok{  dplyr}\SpecialCharTok{::}\FunctionTok{select}\NormalTok{(neighborhood, community, borough, count, proportion) }

\NormalTok{labelled}\SpecialCharTok{::}\FunctionTok{var\_label}\NormalTok{(table\_data}\SpecialCharTok{$}\NormalTok{neighborhood) }\OtherTok{\textless{}{-}} \StringTok{"Neighborhood"}
\NormalTok{labelled}\SpecialCharTok{::}\FunctionTok{var\_label}\NormalTok{(table\_data}\SpecialCharTok{$}\NormalTok{community) }\OtherTok{\textless{}{-}} \StringTok{"Community district"}
\NormalTok{labelled}\SpecialCharTok{::}\FunctionTok{var\_label}\NormalTok{(table\_data}\SpecialCharTok{$}\NormalTok{borough) }\OtherTok{\textless{}{-}} \StringTok{"Borough"}
\NormalTok{labelled}\SpecialCharTok{::}\FunctionTok{var\_label}\NormalTok{(table\_data}\SpecialCharTok{$}\NormalTok{count) }\OtherTok{\textless{}{-}} \StringTok{"Count of gatherings"}
\NormalTok{labelled}\SpecialCharTok{::}\FunctionTok{var\_label}\NormalTok{(table\_data}\SpecialCharTok{$}\NormalTok{proportion) }\OtherTok{\textless{}{-}} \StringTok{"Proportion of gatherings"}

\NormalTok{table\_data }\SpecialCharTok{|\textgreater{}}
\NormalTok{  gt}\SpecialCharTok{::}\FunctionTok{gt}\NormalTok{()}\SpecialCharTok{|\textgreater{}}
\NormalTok{  gt}\SpecialCharTok{::}\FunctionTok{tab\_header}\NormalTok{(}
    \AttributeTok{title =} \StringTok{"Most popular neighborhoods for gatherings"}\NormalTok{,}
    \AttributeTok{subtitle =} \StringTok{"MPX NYC 2022"}
\NormalTok{  ) }\SpecialCharTok{|\textgreater{}}
\NormalTok{   gt}\SpecialCharTok{::}\FunctionTok{fmt\_percent}\NormalTok{(proportion, }\AttributeTok{decimals =} \DecValTok{0}\NormalTok{)}
\end{Highlighting}
...
\begin{longtable*}{lllrr}
\caption*{
{\large Most popular neighborhoods for gatherings} \\ 
{\small MPX NYC 2022}
} \\ 
\toprule
...
{\centering \includegraphics{3_results/3_2__results_contact_venues_files/figure-pdf/unnamed-chunk-6-1.pdf}
...
\chapter{Journeys}\label{sec-movement}}
...
\FunctionTok{make\_plotdata\_movement\_matrix}\NormalTok{() }\SpecialCharTok{|\textgreater{}}
  ...
  \FunctionTok{scale\_size\_mpxnyc}\NormalTok{() }\SpecialCharTok{+}
...
{\centering \includegraphics{3_results/3_3__results_movement_files/figure-pdf/fig-movement-matrix-1.pdf}
...
\FunctionTok{make\_plotdata\_spatial\_concentration\_bar}\NormalTok{() }\SpecialCharTok{|\textgreater{}}
  ...
  \FunctionTok{scale\_alpha\_mpxnyc}\NormalTok{() }\SpecialCharTok{+}
...
{\centering \includegraphics{3_results/3_3__results_movement_files/figure-pdf/fig-spatial-concentration-bar-1.pdf}
...
{\centering \includegraphics{3_results/3_3__results_movement_files/figure-pdf/unnamed-chunk-4-1.pdf}
...
{\centering \includegraphics{3_results/3_4__results_mixing_files/figure-pdf/fig-racegender-mixing-matrix-1.pdf}

}

\caption{\label{fig-racegender-mixing-matrix}Social mixing matrices for
MPX NYC participants by race-gender}
...
{\centering \includegraphics{3_results/3_4__results_mixing_files/figure-pdf/fig-sexorientation-mixing-matrix-1.pdf}
...
{\centering \includegraphics{3_results/3_4__results_mixing_files/figure-pdf/fig-age-mixing-matrix-1.pdf}
...
{\centering \includegraphics{3_results/3_4__results_mixing_files/figure-pdf/unnamed-chunk-5-1.pdf}

}

\end{figure}

\hypertarget{outbreaks-sec-intervention}{%
\chapter{*Outbreaks
\{\#sec-intervention\}}\label{outbreaks-sec-intervention}}
...
\FunctionTok{make\_plotdata\_coverage\_approach\_choro}\NormalTok{() }\SpecialCharTok{|\textgreater{}}
                ...
                \FunctionTok{theme\_mpxnyc\_blank}\NormalTok{(}\AttributeTok{legend.position =} \StringTok{"bottom"}\NormalTok{) }
\end{Highlighting}
\end{Shaded}

\begin{figure}[H]

{\centering \includegraphics{3_results/3_5__results_outbreak_control_files/figure-pdf/fig-hiimpact-communities-map-1.pdf}
...
\FunctionTok{make\_plotdata\_coverage\_approach\_bar}\NormalTok{() }\SpecialCharTok{|\textgreater{}}
  ...
  \FunctionTok{scale\_fill\_mpxnyc}\NormalTok{(}\AttributeTok{name =} \StringTok{""}\NormalTok{, }\AttributeTok{na.translate =} \ConstantTok{FALSE}\NormalTok{, }\AttributeTok{option =} \StringTok{"light"}\NormalTok{)  }\SpecialCharTok{+}
  \FunctionTok{scale\_axis\_mpxnyc}\NormalTok{() }\SpecialCharTok{+}
  \FunctionTok{theme\_mpxnyc\_bar}\NormalTok{(}\AttributeTok{legend.position =} \StringTok{"bottom"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{figure}[H]

{\centering \includegraphics{3_results/3_5__results_outbreak_control_files/figure-pdf/fig-hiimpact-communities-bar-1.pdf}
...
\FunctionTok{make\_plotdata\_net\_impact\_profile}\NormalTok{() }\SpecialCharTok{|\textgreater{}}
  ...
  \FunctionTok{scale\_axis\_mpxnyc}\NormalTok{() }\SpecialCharTok{+}
  \FunctionTok{scale\_x\_continuous\_mpxnyc}\NormalTok{() }\SpecialCharTok{+}
...
{\centering \includegraphics{3_results/3_5__results_outbreak_control_files/figure-pdf/fig-netimpact-1.pdf}
...
{\centering \includegraphics{3_results/3_5__results_outbreak_control_files/figure-pdf/unnamed-chunk-5-1.pdf}
...
\chapter{SSNAC I: Context}\label{sec-appendix-social-spatial}}
...
\hypertarget{integrating-context}{%
\section*{Integrating context}\label{integrating-context}}
\addcontentsline{toc}{section}{Integrating context}

\markright{Integrating context}
...
there is only one infected person. Since colds are transmitted through
close contact among infected and susceptible people, we can
conceptualize the risk of an individual as a characteristic of the
physical context they share with other (possibly infected) people.

The simplest approach (first frame, Figure~\ref{fig-context}) would be
to assign the average risk of acquiring a cold to each individual based
on knowledge about the total number of susceptible people, and the
number of infected people. The problem with this strategy, though, would
be that those who are more likely to interact closely with the infected
person have higher risk, and not all people are equally likely to
interact.

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{plot\_context\_figure}\NormalTok{()}
\end{Highlighting}
\end{Shaded}
...
We could improve on this analysis using a \textbf{social network
analysis} (second frame, Figure~\ref{fig-context}). In this approach, we
would use knowledge about relationships to gain a more granular
understanding of risk. We could assume, for instance, that people who
are in relationships that cause them to physically interact have higher
risk than those who are not in such relationships. We could further
assign different levels of risk to different connected components of the
network. In Figure~\ref{fig-context}, relationships are shown using
lines connecting the dots.

An \textbf{ecologic} \textbf{analysis} (third frame,
Figure~\ref{fig-context}) would use knowledge about patterns of
interaction in space to group people into categories. We could divide
people into households, for instance. Assuming that people physically
interact with household members significantly more than they do with
non-members, households could be assigned differing levels of risk. In
Figure~\ref{fig-context}, households are marked using a fence
surrounding each household.

A \textbf{spatial analysis} (fourth frame, Figure~\ref{fig-context})
...
analysis we assume that some pairs of categories influence each other as
a result of their adjacency in physical space. We could assume, for
example, that neighboring households influence each other's risk since
their members interact. In Figure~\ref{fig-context}, adjacency is shown
as a gap in the boundary separating households.

A \textbf{SSNAC analysis} (final frame, Figure~\ref{fig-context}) would
integrate knowledge about patterns of interaction between individuals,
within spatial units, and across spatial units to gain a granular
understanding of risk. We could assume that the risk of acquiring a cold
is a function of i. interactions with one's own household members, ii.
interactions with members of neighboring households, iii. interactions
in personal relationships within and across households.

\hypertarget{social-and-spatial-analysis-with-causal-interpretation-ssnac}{%
\section*{Social and spatial analysis with causal interpretation
(SSNAC)}\label{social-and-spatial-analysis-with-causal-interpretation-ssnac}}
\addcontentsline{toc}{section}{Social and spatial analysis with causal
interpretation (SSNAC)}

\markright{Social and spatial analysis with causal interpretation
(SSNAC)}

The SSNAC framework is an approach to collecting, analyzing and
interpreting data which explicitly accounts for causal relations among
people, among places, and across people and places. It is applicable in
two situations: when investigators want to describe patterns in how
people interact with each other and/or with shared spaces; or when
investigators are interested in understanding or intervening on the
causal system formed by these components.

For the descriptive interest, it is necessary to have data on
individuals and spatial units they co-inhabit. For richer analyses, data
on relationships among individuals and on relationships between spatial
units could be incorporated. For the causal interest, it is necessary
for investigators to additionally have a theory or set of theories about
the causal process under investigation. At minimum, theory should
clarify:
...
\subsection*{The data are a network}\label{the-data-are-a-network}}
\addcontentsline{toc}{subsection}{The data are a network}

The central data structure in a SSNAC is the data graph.
Figure~\ref{fig-ssnac} depicts the final frame of
Figure~\ref{fig-context}, showing households as nodes in a network,
connecting neihgboring households with a network edge, and connecting
individuals with the households they belong to. In a data graph, study
data are stored as node or edge attributes. Person nodes might have
attributes like age and race whereas place nodes could have attributes
like density or surface area. An edge linking a person to a place might
have an attribute conveying how the person relates to the place
(e.g.~home or work) and an edge linking a person to a person might have
attributes related to the type of relationship (e.g.~friend, sex
partner), etc.

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{data\_snac\_example}\NormalTok{() }\SpecialCharTok{|\textgreater{}}
...
{\centering \includegraphics{A_ssnac1_social_spatial_context/A__ssnac1_social_spatial_context_files/figure-pdf/fig-ssnac-1.pdf}

}

\caption{\label{fig-ssnac}SSNAC data graph encoding people, places, and
spatial and social connections}

\end{figure}

\hypertarget{the-network-is-a-causal-structure}{%
\subsection*{The network is a causal
structure}\label{the-network-is-a-causal-structure}}
\addcontentsline{toc}{subsection}{The network is a causal structure}

The data graph is a starting point for constructing a causal model, but
it is not sufficient. Depending on the causal question, some edges in
this graph are relevant and some are not. For example, if we were
interested in a rumor network, we might decide to depict only people and
not places in the causal graph. We might also choose to ignore the
neighbor relation and focus only on the friend and resident relations if
it is the case that rumors are transmitted within households and across
friendships but not to neighbors.

If we were interested in COVID during a stay-at-home order, by contrast,
we might keep the neighbor and resident edges and drop the friend edges.
In either case, it is necessary to make changes to the data graph so
that the definitions for nodes and edges are compatible with the theory
guiding the analysis.

\hypertarget{new-causal-structures-open-new-causal-enquiries}{%
\subsection*{New causal structures open new causal
enquiries}\label{new-causal-structures-open-new-causal-enquiries}}
\addcontentsline{toc}{subsection}{New causal structures open new causal
enquiries}

The SSNAC framework opens up the potential to investigate causal queries
that more closely resemble the program decisions that public health
leaders have to make when faced with health challenges. The four
standard SSNAC causal queries focus on node-level outcomes in the SNACC
framework.

\textbf{Person-to-place intervention}: Evaluating the causal impact of a
person-level intervention on place-level outcomes for some defined set
of people and some defined set of places. e.g.~What is the impact of
vaccinating college-aged adults in New York City on mpox incidence in
Washington Square (a favorite hangout spot for NYU students)?

\textbf{Person-to-person intervention}: Evaluating the impact of a
person-level intervention on person-level outcomes for some defined sets
of people. e.g.~How does vaccinating all cisgender white men in New York
City affect mpox incidence among Black and Latinx cisgender black men in
New York City?

\textbf{Place-to-place intervention}: Evaluating the impact of a
place-level intervention on place-level outcomes for some defined sets
of places. e.g How does vaccinating Manhattan affect mpox incidence in
Brooklyn?

\textbf{Place-to-person intervention}: Evaluating the impact of a
place-level intervention on person-level outcomes for some defined set
of people and some defined set of places. e.g.~How does vaccinating
Harlem affect mpox incidence among cisgender black men in New York City
...
emerge as a result of the pattern of interactions among nodes. We
investigate such a query in Appendix~\ref{sec-causal-interpretation}.

\hypertarget{sec-appendix-network-analysis}{%
\chapter{SSNAC II: Analysis}\label{sec-appendix-network-analysis}}
...
{\centering \includegraphics{B_ssnac2_network_analysis/B__ssnac2_network_analysis_files/figure-pdf/fig-example-participant-place-1.pdf}
...
{\centering \includegraphics{B_ssnac2_network_analysis/B__ssnac2_network_analysis_files/figure-pdf/fig-example-participant-1.pdf}
...
{\centering \includegraphics{B_ssnac2_network_analysis/B__ssnac2_network_analysis_files/figure-pdf/fig-example-community-1.pdf}
...
nodes which are included in some defined subset \(A\) of nodes. We thus
define conditional participant social reach (\(R_{i \rightarrow A}\)),
conditional participant spatial reach (\(S_{i \rightarrow A}\)),
conditional community district social reach (\(r_{j \rightarrow A}\)),
and conditional community district spatial reach
(\(s_{j \rightarrow A}\)).
...
\[
R_{i  \rightarrow A} = \sum_{j \in N_p}{\psi_p (i,j)I_{x \in A}(x = j) }
\]
...
\[
S_{i \rightarrow A} = \sum_{j \in N_c}{\psi (i,j)I_{x \in A}(x = j)}
\]
...
\[
r_{j  \rightarrow A} = \sum_{i \in N_p}{\psi (i, j)I_{x \in A}(x = i)}
\]
...
\[
s_{j \rightarrow A} = \sum_{i \in N_c}{\psi_c (i,j)I_{x \in A}(x = i)}
\]
...
The spatial concentration chart plots the spatial concentration
proportion for each community district.
...
\hypertarget{sec-causal-interpretation}{%
\chapter{SSNAC III: Causality}\label{sec-causal-interpretation}}
...
In this appendix we elaborate the ``causal interpretation'' aspect of
the Social and Spatial Network Analysis with Causal interpretation
(SNACC) framework. The SSNAC framework can be used to guide the
collection, management, analysis, and causal interpretation of data. It
offers guidance on conceptualizing and organizing spatial and social
data as networks (Appendix~\ref{sec-appendix-social-spatial}) and
provides tools for descriptive and causal analysis
(Appendix~\ref{sec-appendix-network-analysis}).

We define SSNAC as a set of straight-forward extensions of the currently
dominant framework for conducting causal inference with observational
data in epidemiology. We begin by giving an overview of the dominant
approach, which we will call ``classic causal inference''. We extend
this system to situations where data are characterized by
interdependence of observational units rather than independence, naming
this system ``social network analysis with causal interpretation''.
Finally, we accommodate social and spatial data simultaneously in the
SSNAC framework. In each case, we describe the assumptions and
implications of each system, illustrating with worked examples.

Classic causal inference, as elaborated by Robins (1986), Hernán and
Robins (2018), and others helps us to sharpen epidemiologic reasoning by
...
about the likely effect of some hypothetical intervention on some
outcome in a defined population. They define a set of causal estimands -
measures that quantify the impact of the intervention of the outcome,
then through transparent statistical assumptions, express those
estimands in terms of quantities we can calculate by studying the
...
would change if we implemented such an intervention. We can even ask
about the extent of change and estimate that using data that have
already been observed. The classic causal inference framework allows
this by:

\begin{itemize}
\item
  making causal assumptions about the relationship between healthcare
  access, diabetes prevalence, and any factors that determine both
  access and prevalence
\item
  mathematically stating the hypothetical quantities we are interested
  in and expressing them in terms of quantities we can estimate
\item
  estimating those quantities using data we have or can collect
\end{itemize}

\textbf{Independence and homogeneity}
...
groups of observational units. This assumption is usually made with the
goal of simplifying the mathematics involved in estimating quantities
from data, but it often has profound implications for how we think about
the causal process under study.

In our opening example, the independence assumption holds that one
person's access to healthcare does not affect any other person's
diabetes status. It would imply that the diabetes status for a married
man who does not know how to cook, for instance, would not at all be
affected by improvements in health literacy experienced by his wife who
cooks for the household. The assumption of independence runs counter to
our intuitions about social life. Humans are networked - our lives are
defined by our relationships with others.

Along with the independence assumption, practitioners of classical
causal inference also make assumptions about the homogeneity of the
causal process under study. An example to clarify: Consider two people,
Tumi and Thabo, who each toss the same coin once at time 1 and again at
time 2. There are several ways of parsing this situation into
``experiment'' and ``repetition''. We could say that we are conducting:
...
``Thabo tosses a coin, then Tumi tosses the coin, then Thabo tossess the
coin, then Tumi tosses the coin''. Whereas in the above listing, by
virtue of calling them separate experiments, we implicitly assumed that
each coin toss was self-contained, in this latest iteration, we
accommodate the possibility that the results of the coin tosses might be
correlated.
...
Without stating the experiment as this sequence, however, we have no
framework with which to ask whether Thabo and Tumi's coin tosses are
causally related.
...
that it is often presented as a necessary condition for doing valid
...
\textbf{Towards a causal inference of dependent happenings}

Our goal in this appendix and project is to offer ideas and tools for
causal inference that allow researchers to learn from interdependence
rather than avoid it in pursuit of clear causal reasoning. We can have
both clear reasoning and an understanding enriched by the analysis of
dependent happenings.

In crude terms, our approach involves moving to a framework where, as a
rule, we consider Thabo and Tumi's coin tosses one experiment, rather
...
some of the entries in that vector are correlated as a result of being
part of the same causal structure.

But why would a researcher want to do that when our tools for
statistical modelling require as many repetitions as possible for valid
inferences? How can we quantify uncertainty when we always have a sample
size of 1? To answer the first question - we take this approach because
it allows us to investigate and quantify causal interference. Regarding
the second, we advocate a shift in perspective: we re-frame the task of
statistical inference from that of learning from a growing collection of
identically distributed random vectors, to that of learning from a
growing random vector whose entries are related by a causal structure
that is identical across groups of entries.

We avail ourselves of asymptotic theory by making the assumption of
no-spillover between observations, which is a weaker version of the
independence between observational units assumption. We further assume
that the causal relationships between and within individuals are
homogeneous in some way, allowing our estimates to benefit from having
more observational units, even if they collectively comprise one
realization of the statistical model.

In the sections that follow, we describe classic causal inference,
social network analysis with causal interpretation, and the SSNAC
framework, each time discussing the assumptions, causal model, causal
estimands, and illustrating a causal identification strategy for the
framework. Finally, we describe the MPX NYC outbreak response analysis
presented in the main report using the SSNAC framework.
...
Say \emph{Person 1} has job status \(L_1\), \emph{Person 2} has job
status \(L_2\) and \emph{Person 3} has job status \(L_3\). Each of these
variables can only take on two values: ``employed vs.~unemployed''.
Similarly we define \(A_i\) as \emph{Person \(i\)'s} dress (formal
vs.~informal), and \(Y_i\) as their job offers (many vs.~few). The job
status measurement (\(L_i\)) precedes dress (\(A_i\)) in time, and dress
precedes job offers (\(Y_i\)).

In the next few sections, we will use this example to describe the
assumptions made under classical causal inference. First we give an
overview of the ideas and tools we need to make that description: DAGs
and SWIGs.

\hypertarget{causal-reasoning-with-dags-and-swigs}{%
\subsection*{Causal reasoning with DAGs and
SWIGs}\label{causal-reasoning-with-dags-and-swigs}}
\addcontentsline{toc}{subsection}{Causal reasoning with DAGs and SWIGs}

\hypertarget{finest-fully-randomized-causally-interpretable-structured-tree-graph}{%
\paragraph{Finest, fully randomized, causally interpretable structured
tree
graph}\label{finest-fully-randomized-causally-interpretable-structured-tree-graph}}

At its core, the system we use in epidemiology to reason about
counterfactual quantities such as causal effects is a statistical model
called the Finest Fully Randomized Causally Interpretable Structured
Tree Graph (FFRCISTG). In this current section
(Section~\ref{sec-classic-causal-inference}) we describe the assumptions
of the FFRCISTG model using language that will aid their extension in
Section~\ref{sec-network-causal-inference}. This note is a brief
overview of the FFRCISTG.

A Finest, fully randomized, causally interpretable structured tree graph
(FFRCISTG) is a collection of non-parametric structural equation models
which mimic the causal process that produced the variables under
investigation. At the very least it is a collection of two NPSEMs -- a
model of the observed data (observed data model) and a model of what the
data would be under some intervention (counterfactual data model). The
most common type of intervention in this framework is one which sets the
value of one or more variables in the system (the exposures) with the
goal of influencing the value of some set of variables in the system
(the outcomes). The counterfactual data model is used to calculate
quantities like causal effects. The observed data model is used to
express those quantities in terms of statistics that can be estimated

A non-parametric structural equation model with independent errors is a
system of equations that relates all the entries in a given
vector-valued random variable to each other. It does so without
...
that are produced by equations higher up in the list serve as inputs to
...
\(U_*\) represent error terms -- random variables that are independent
of each other within and across observational units.

Consider a FFRCISTG modelling an intervention where we set formal dress
\(A_1\) to \(a_1\), \(A_2\) to \(a_2\), and \(A_3\) to \(a_3\). The
observed data model would be:
...
\begin{equation}\protect\hypertarget{eq-fullly-connected-swig}{}{
\begin{aligned}
L_1 &= f_{L_1}(U_{L_1}) \\
L_2 &= f_{L_2}(U_{L_2}, L_1) \\
L_3 &= f_{L_3}(U_{L_3}, L_1, L_2 ) \\
...
\end{aligned}
}\label{eq-fullly-connected-swig}\end{equation}

\marginnote{\begin{footnotesize}

Notice how in Equation~\ref{eq-fullly-connected-swig}, the values set
under the intervention supercede the values that would have been
generated had the experimenter not intervened. The former values are
used in subsequent equations in the model whereas the latter values are
not.

\end{footnotesize}}

\emph{Causal DAGs and SWIGS}

A causal directed acyclic graph (DAG) is a graph whose nodes represent
the variables in an observational non-parametric structural equation
model with independent errors. Directed edges point from the arguments
to the result of each structural equation.
...
{\centering \includegraphics{C_sscan3_causal_interpretation/C__sscan3_causal_interpretation_files/figure-pdf/fig-fullly-connected-dag-1.pdf}
...
A single world intervention graph (SWIG) is a graph whose nodes
represent the variables in a counterfactual non-parametric structural
equation model with independent errors. Directed edges point from the
arguments to the result of each structural equation.

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{make\_swig}\NormalTok{(}\StringTok{"fully\_connected"}\NormalTok{)      }\SpecialCharTok{|\textgreater{}} 
  \FunctionTok{plot\_swig}\NormalTok{(}\AttributeTok{node\_radius =} \FloatTok{0.2}\NormalTok{) }\SpecialCharTok{+} 
  \FunctionTok{scale\_fill\_mpxnyc}\NormalTok{(}\AttributeTok{option =} \StringTok{"light"}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{theme\_mpxnyc\_blank}\NormalTok{()}
\end{Highlighting}
...
{\centering \includegraphics{C_sscan3_causal_interpretation/C__sscan3_causal_interpretation_files/figure-pdf/fig-fullly-connected-swig-1.pdf}

}

\caption{\label{fig-fullly-connected-swig}Fully connected SWIG}

\end{figure}

\hypertarget{model-restrictions}{%
\paragraph{Model restrictions}\label{model-restrictions}}

Whereas parametric models are characterized by their probability density
functions, non-parametric models of this type are usually characterized
by their conditional independencies. For instance, if we add the
restriction that all the measures are independent (or equivalently, that
the DAG has no arrows), the resulting model would not be able to
represent data from a causal system where some of the measures are
correlated. By adding the restriction of independence, we would be
eliminating the correlated data distribution from consideration as a
possibility for how the data we are modelling are truly distributed.

It is possible to check whether two variables in a NPSEM are
(conditionally) independent using the structural equations themselves by
checking whether both variables are a function of the same error term
\(U_*\). DAGs and SWIGs are useful because they simplify this task by
encoding the statistical relationships through a set of graphical rules
called d-separation.\footnote{\textbf{D-separation} is a set of rules
  for translating DAGs into conditional independence statements. A pair
  of nodes is d-separated if all the paths linking them are closed or if
  there is no path that links them.

  A \textbf{path} is a sequence of nodes that starts with a given origin
  node and ends with a terminal node. Consecutive entries in the
  sequence are connected in the DAG. A path of length one is open by
  definition. A path of length two or more is open if all its triangles
  are open.

  A \textbf{triangle} is a sub-sequence of a path which contains three
  consecutive nodes from the path. A canonical triangle is one in which
  we do not condition on the apex node. A conditional triangle is one in
  which we do condition.

  A \textbf{fork} is a triangle whose apex variable has two outgoing
  edges. A canonical fork is open and a conditional fork is closed. The
  apex variable of a fork is commonly referred to as a ``confounder''.

  A \textbf{spoon} is a triangle whose apex variable has two incoming
  edges. A canonical spoon is closed and a conditional spoon is open.
  The apex variable of a fork is commonly referred to as a ``collider''.

  A \textbf{chopstick} is a triangle whose apex variable has one
  incoming edge and one outgoing edge. A canonical chopstick is open and
  a conditional chopstick is closed. The apex variable of a chopstick is
  commonly referred to as a ``mediator''.} Two variables in an NPSEM are
conditionally independent if they are d-separated in the causal DAG
representing the NPSEM.

Restrictions are crucial: to fully characterize the NPSEM specified in
Equation~\ref{eq-fullly-connected-dag} using observed data, we would
need to estimate 9 functions using one observation of the 9-dimensional
joint distribution that gave rise to the participants' observations.
...
In practice, we simplify this system of equations by making assumptions
that leave us with fewer functions to estimate and more observations per
function. These assumptions are called model restrictions because they
restrict the set of distributions that could possibly be represented by
the NPSEM.
...
\hypertarget{independence-i-no-spillover}{%
\paragraph{Independence I: No
spillover}\label{independence-i-no-spillover}}
...
{\centering \includegraphics{C_sscan3_causal_interpretation/C__sscan3_causal_interpretation_files/figure-pdf/fig-no-spillover-dag-1.pdf}
...
\hypertarget{independence-ii-no-interference}{%
\paragraph{Independence II: No
interference}\label{independence-ii-no-interference}}
...
\hypertarget{homogeneity-i-structural-homogeneity}{%
\paragraph{Homogeneity I: Structural
homogeneity}\label{homogeneity-i-structural-homogeneity}}
...
Equation~\ref{eq-independence-dag} and we keep all 9 nodes in the DAG on
Figure~\ref{fig-independence-dag}. This is to remind us that though our
...
{\centering \includegraphics{C_sscan3_causal_interpretation/C__sscan3_causal_interpretation_files/figure-pdf/fig-independence-dag-1.pdf}
...
To ensure that counterfactual outcomes are well-defined for all
experimental units, we make the assumption of positivity:
...
{\centering \includegraphics{C_sscan3_causal_interpretation/C__sscan3_causal_interpretation_files/figure-pdf/fig-independence-swig-1.pdf}
...
observed rather than counterfactual model. It is sufficient to identify
the quantity:

\marginnote{\begin{footnotesize}

\textbf{Notes on identification}

\textbf{A}. In the first line, we use the law of iterated expectations.

\textbf{B}. In the second line, we use the definition for counterfactual
outcome \(Y_i^{(a)}\) as specified in
Equation~\ref{eq-independence-swig}.

\textbf{C}. In the third line, we use the fact that when \(L_i\) is
fixed, \(f_{Y}(U_{Y_i} , L_i, a)\) is a function of random variable
\(U_{Y_i}\) and \(A_i\) is a function of \(U_{A_i}\). Since \(U_{Y_i}\)
and \(U_{A_i}\) are independent by definition of the NPSEM-IE,
\(f_{Y}(U_{Y_i} , L_i, a) \perp A_i | L_i\).

\textbf{D}. In the fourth line we consider the expected value of
\(f_{Y}(U_{Y_i} , L_i, a)\) when \(A_i = a\), in particular

\textbf{E}. In the fifth line, we use the definition for observed value
\(Y_i\) according as specified in Equation~\ref{eq-independence-dag}.
...
We can estimate \(E[Y_i | L_i = l, A_i = a]\) non-parametrically by
calculating the average of observations \(Y_i\) for which \(L_i = l\)
and \(A_i = a\). These estimates improve as \(n \rightarrow \infty\).

\hypertarget{assumptions-revisited}{%
\subsection{Assumptions, revisited}\label{assumptions-revisited}}

Typically, there are three assumptions which are usually stated as
necessary conditions for valid causal inferences: conditional
exchangeability,
...
Conditional exchangeability is a consequence of the independence of
errors in the NPSEM-IE (see notes on identification assumptions above).
Consistency is a consequence of the fact that a) the structural
equations for the observed model are identical with the equations for
the counterfactual model, b) the error terms for each individual \(i\)
in the counterfactual model are equal to the error terms in the observed
model. i.e.~\(U_{Y_i}\) has the same value in
Equation~\ref{eq-independence-swig} as it does in
Equation~\ref{eq-independence-dag}. Unlike the first two, the assumption
of positivity is not given by the definition of the NPSEM. We make it in
defining the structural equation models for counterfactual variables
(Equation~\ref{eq-independence-swig}).
...
\item
  How is the SUTVA assumption encoded in this model?
\end{enumerate}

\hypertarget{sec-network-causal-inference}{%
\section{Social network analysis}\label{sec-network-causal-inference}}

\hypertarget{example-2-marriage-jobs}{%
\subsection*{Example 2: Marriage \&
Jobs}\label{example-2-marriage-jobs}}
\addcontentsline{toc}{subsection}{Example 2: Marriage \& Jobs}

We mentioned that in our study sample \emph{Person 1} and \emph{Person
2} are married to each other, and \emph{Person 1} and \emph{Person 3}
are new colleagues. In the causal system connecting their job status,
dress and job offers, an argument could be made that \emph{Person 1}'s
job status affects \emph{Person 2}'s dress, since they pool their
salaries as a married couple. It could also be argued that \emph{Person
1}'s job status affects \emph{Person 2}'s job opportunities if having a
job brings the married couple into contact with other married couples
who are connected to job markets.
...
\emph{Person 2} enjoys among potential colleagues derives from their
spouse's appearance. Symmetrically, \emph{Person 2}'s measures could
influence \emph{Person 1}'s.
...
between persons \emph{1} and \emph{2}, we outline some definitions which
we will need to describe the network causal inference model. Before
that, we will briefly a new structure - the data graph - which we will
also need for the network model.

\hypertarget{modelling-interference-using-exposure-mappings}{%
\subsection*{Modelling interference using exposure
mappings}\label{modelling-interference-using-exposure-mappings}}
\addcontentsline{toc}{subsection}{Modelling interference using exposure
mappings}

Say we have a non-parametric structural equation model with measures
\(\mathbf{Y}, \mathbf{X}_t \in \{0,1\}^n\) with \(t \in [1,K]\).
\(f_{Y_i}\) is the structural equation for \(Y_i\), and \(U_{Y_i}\) the
error term for that equation. We define the graph
\(\mathscr{G}(\mathscr{N}, \psi)\) as an ordered pair whose first entry
is a set containing \(n\) nodes, each representing an observational unit
in the study. The second entry is an adjacency function
\(\psi(i,j) \in \mathbf{R}^+_0\) which gives the weight between nodes
\(i\) and \(j\). Weights are positive real numbers when the nodes are
connected in \(\mathscr{G}\) and zero otherwise. We further define the
``network exposure units'' for node \(i\) as

\[
\mathscr{N_i} = \{j \in \mathscr{N}: \psi(i, j) > 0\}
\]

\(\mathbf{W}_i\) is the ``network weights'' for node \(i\). It is a
vector of length \(|\mathscr{N}_i|\) whose \(j^{th}\) entry is
\(\psi(i,j)\). \(X_i\) is the \(i^{th}\) entry of vector \(\mathbf{X}\)
and \(\mathbf{X}_{\mathscr{N_i}}\) is the sub-vector of \(\mathbf{X}\)
which contains entries related to person \(i\)'s network exposure units.
Where the original vector already has a subscript, we append onto that
subscript \(i\) and \(\mathscr{N_i}\), respectively. For instance,
\(X_{ti}\) is the \(i^{th}\) entry of \(\mathbf{X}_t\) and
\(\mathbf{X}_{t\mathscr{N_i}}\) is the sub-vector relating to node
\(i\)'s network exposure units. \(\mathbf{M}\) is a permutation matrix
-- a square matrix whose row and column margins all equal 1, and whose
entries are either zero or one. Pre-multiplying a vector with a
conformable permutation matrix permutes the order of the entries in the
vector.

\hypertarget{definition-1-exposure-mapping}{%
\paragraph{\texorpdfstring{\textbf{Definition 1: Exposure
mapping}}{Definition 1: Exposure mapping}}\label{definition-1-exposure-mapping}}

We define the ``exposure mapping for \(Y_i\)'' as a function
\(g_i \in \mathbf{R}^{2K}\) such that:
...
\paragraph{\texorpdfstring{\textbf{Definition 2:
Separability}}{Definition 2: Separability}}\label{definition-2-separability}}
...
where \(g_0 = [X_{1i}, ..., X_{Ki}]^T\) and \(g_{ti} \in \mathbf{R}\).
We call the functions \(g_{ti}\) component functions of exposure mapping
\(g_i\).

\hypertarget{definition-3-loyalty}{%
\paragraph{\texorpdfstring{\textbf{Definition 3:
Loyalty}}{Definition 3: Loyalty}}\label{definition-3-loyalty}}

We say a component function \(g_{ti}\) is loyal to graph \(\mathscr{G}\)
if for \(\mathbf{X}_t,\mathbf{X}'_t \in \{0,1\}^n\)
...
\hypertarget{definition-4a-positional-invariance}{%
\paragraph{\texorpdfstring{\textbf{Definition 4a: Positional
invariance}}{Definition 4a: Positional invariance}}\label{definition-4a-positional-invariance}}

We say component functions \(g_{ti}\) are positionally invariant if for
\(i,j \in \mathscr{N}\), \(\mathbf{X},\mathbf{X}' \in \mathbf{R}^n\) and
conformable permutation matrix \(\mathbf{M}\):

\[
\begin{equation}
\mathbf{X}_{\mathscr{N}_i} = \mathbf{M} \mathbf{X'}_{\mathscr{N}_j} \text{ and } \mathbf{W}_{i\mathscr{N}_i} =  \mathbf{M} \mathbf{W}_{j\mathscr{N}_j}\\
\implies \\
g_{ti}(\mathbf{X}) = g_{tj}(\mathbf{X'}) 
\end{equation}
\]

\hypertarget{definition-4b-network-exposure-mapping}{%
\paragraph{\texorpdfstring{\textbf{Definition 4b: Network exposure
mapping}}{Definition 4b: Network exposure mapping}}\label{definition-4b-network-exposure-mapping}}

If functions \(g_{ti}\) is positionally invariant with respect to
\(\mathscr{G}\) we say it is a ``network exposure mapping for
\(\mathbf{Y}\) with respect to \(\mathbf{X}_t\) in \(\mathscr{G}\)''.
The value of the function is called the ``network exposure value for
\(Y_i\) with respect to \(\mathbf{X}_t\) in \(\mathscr{G}\)''.
Furthermore,
...
\paragraph{\texorpdfstring{\textbf{Definition 5:
Boundedness}}{Definition 5: Boundedness}}\label{definition-5-boundedness}}

We say that a network exposure mapping \(g_{t}\) is bounded if all
...
and \(|\mathbf{D}| < c\) for some constant \(c\), where \(c\) is not a
function of the number of interconnected units in graph \(\mathscr{G}\).

The assumption of boundedness can obtain in three different ways. It
could be as a result of restrictions on the structure of the data graph.
It could be the case that in the network of interest, there is an upper
bound on node degree. For example, if the only relationships of interest
between individual participants were marriages, each node would have a
degree of either 0 or 1, meaning the count of alters with
\(X_{tj} = 1, j \in \mathscr{N}_i\) is either zero or one.

It might obtain via restrictions of the value of \(\mathbf{X}_t\)
itself. It might be the case, for instance, that the count of \(i\)'s
for which \(X_{ti}=1\) is very low so that on average, the count of
alters for which \(X_{tj} = 1, j \in N_i\) will also be low.

Finally, boundedness could obtain as a result of the functional form of
\(g_t\) itself. This might happen if \(g_t\) models a network exposure
which has a ceiling effect in its dose-response curve with the outcome
of interest. For example, if \(X_t\) indicates ownership of a car, and
...
shifts the outcome of interest. For most health applications, its likely
that beyond some number of friends (say 10) who have a car, an
additional friend who gets a car will not change the outcome.

\hypertarget{assumptions}{%
\subsection{Assumptions}\label{assumptions}}

\hypertarget{independence-i-no-spillover-1}{%
\paragraph{Independence I: No
spillover}\label{independence-i-no-spillover-1}}
...
\hypertarget{independence-ii-network-interference}{%
\paragraph{Independence II: Network
interference}\label{independence-ii-network-interference}}
...
{\centering \includegraphics{C_sscan3_causal_interpretation/C__sscan3_causal_interpretation_files/figure-pdf/fig-interference-dag-1.pdf}
...
2}'s, and neither are connected with \emph{Person 3}'s.
...
DAG is loyal to data graph \(\mathscr{G}\) which encodes the causally
relevant relationships between observational units in the sample. In our
example
...
\hypertarget{homogeneity-i-relational-homogeneity}{%
\paragraph{Homogeneity I: Relational
homogeneity}\label{homogeneity-i-relational-homogeneity}}
...
\hypertarget{homogeneity-iii-structural-homogeneity}{%
\paragraph{Homogeneity III: Structural
homogeneity}\label{homogeneity-iii-structural-homogeneity}}
...
\hypertarget{homogeneity-iv-boundedness}{%
\paragraph{Homogeneity IV:
Boundedness}\label{homogeneity-iv-boundedness}}
...
infer something about the sample, not any one individual in it.
...
{\centering \includegraphics{C_sscan3_causal_interpretation/C__sscan3_causal_interpretation_files/figure-pdf/fig-homogenous-dag-1.pdf}
...
\[
0 < P[A_i = a_i,  h_Y (\mathbf{A}, i) =  h_Y(\mathbf{a}, i)| L_i,g_{A}(\mathbf{L}, i) ] < 1
...
{\centering \includegraphics{C_sscan3_causal_interpretation/C__sscan3_causal_interpretation_files/figure-pdf/fig-homogenous-interference-swig-1.pdf}
...
L_i &= f_{L}(  U_{L_i}  ) \\
A_i &= f_{A}( U_{A_i}, L_i, g_{A}(\textbf{L}, i) ) \\
Y_i^{(a_i,\textbf{a}_{\mathscr{N}_i} )} &= f_{Y}(U_{Y_i}, L_i,  a_i,  g_{Y}(\textbf{L}, i),      h_{Y}(\textbf{a}, i)         ) \\
\end{aligned} 
...
\textbf{A}. In the first line, we use the loyalty of the exposure
mapping for \(Y_i^{(a_i, \mathbf{a}_{\mathscr{N}_i})}\) to data graph
\(\mathscr{G}\) to simplify the conditioning statement.

\textbf{A}. In the second line we use the definition of counterfactual
variable \(Y_i^{(a_i, \mathbf{a}_{\mathscr{N}_i})}\) given in
Equation~\ref{eq-homogenous-dag}.

\textbf{B}. In the third line, we use the fact that given fixed
\(\mathbf{L}\),
\(f_{Y}(U_{Y_i}, L_i, a_i, g_{Y}(\textbf{L}), h_{Y}(\textbf{a}))\) is a
function of random variable \(U_{Y_i}\). Since \(U_{Y_i}\) is
independent of \(U_{A_i}\) as well as \(U_{A_j}\) for
\(j \in \mathscr{N}_i\), \(f_{Y}(...) \perp \mathbf{A} | \mathbf{L}\).

\textbf{C}. In the final line, we use the definition of observed
variable \(Y_i\) given in Equation~\ref{eq-homogenous-dag}.
...
\begin{aligned}
 ...
 &=  E[Y_i^{(a_i, \mathbf{a}_{\mathscr{N}_i})}                                  | L_i , g_Y(\mathbf{L}, i)]  \\
 &=  E[f_{Y}(U_{Y_i}, L_i,  a_i, g_{Y}(\textbf{L}, i),  h_{Y}(\textbf{a}, i))   | L_i , g_Y(\mathbf{L}, i)]  \\ 
 &=  E[f_{Y}(...)                                                               | L_i , g_Y(\mathbf{L}, i), A_i = a_i, h_Y(\textbf{A}, i) = h_Y(\textbf{a}, i)] \\
 &=  E[Y_i                                                                      | L_i , g_Y(\mathbf{L}, i), A_i = a_i, h_Y(\textbf{A}, i) = h_Y(\textbf{a}, i)] \\
\end{aligned}
\]
...
\[
E[ \frac{1}{n} \sum_i  E[Y_i |  L_i , g_Y(\mathbf{L}, i), A_i = a_i, h_Y(\textbf{A}, i) = h_Y(\textbf{a}, i) ]]
\]
...
We can estimate
\(E[Y_i | L_i = l , g_Y(\mathbf{L}, i) = l', A_i = a, h_Y(\textbf{A}, i) = a' ]\)
non-parametrically by calculating the average of observations \(Y_i\)
for which \(L_i = l\), \(A_i = a\), \(g_Y(\mathbf{L}, i) = l'\), and
\(h_Y(\textbf{A}, i) = a'\). The assumption of boundedness guarantees
that as \(n \rightarrow \infty\), we will get better estimates for each
cell rather than a proliferation of cells to estimate.

\hypertarget{assumptions-revisited-1}{%
\subsection{Assumptions, revisited}\label{assumptions-revisited-1}}

Although we state the assumption of no-spillover as a starting point for
the SSNAC model, it is possible to conduct valid causal inference using
network data even if that assumption does not hold. In a study on
physical function among older South Africans, Makofane et. al used a
chain graph rather than a DAG to represent such data. Chain graph's are
DAGs that specify a correlation structure among contemporaneous
variables. These are assumed to arise from a conditional markov field.
The undirected edges in this field represent causal processes that flow
in both directions along the edge, reaching some equilibrium.

Though it is beyond the scope of this project, it is possible to
articulate a causal model in the face of networked spillover and
interference. In some cases, point estimates from an analysis which
ignores spillover are unbiased while the standard errors from such an
analysis are biased.
...
\item
  How does the assumption of boundedness guarantee that as
  \(n \rightarrow \infty\), we will get better estimates of the average
  counterfactual values rather than a proliferation of counterfactual
  values to estimate?
\item
  Write an example of a network exposure mapping that is not separable.
\end{enumerate}

\hypertarget{sec-ssnac}{%
\section{Social \& spatial analysis}\label{sec-ssnac}}

\hypertarget{example-3-vaccination}{%
\subsection*{Example 3: Vaccination}\label{example-3-vaccination}}
\addcontentsline{toc}{subsection}{Example 3: Vaccination}
...
\hypertarget{introducing-dangs-and-swings}{%
\subsection*{Introducing DANGs and
SWINGs}\label{introducing-dangs-and-swings}}
\addcontentsline{toc}{subsection}{Introducing DANGs and SWINGs}
...
Equation~\ref{eq-homogenous-dag} and shown in
Figure~\ref{fig-homogenous-dag}. The structural part of the causal DANG
shows the causal structure within individuals. It is a DAG of
vector-valued variables \(\mathbf{L}\), \(\mathbf{A}\), and
...
observational units have correlated measures with each other. For
instance, in Figure~\ref{fig-causal-dang-instructions} we see that
individuals 1 and 2 are related.
...
{\centering \includegraphics{C_sscan3_causal_interpretation/C__sscan3_causal_interpretation_files/figure-pdf/fig-causal-dang-instructions-1.pdf}
...
{\centering \includegraphics{C_sscan3_causal_interpretation/C__sscan3_causal_interpretation_files/figure-pdf/fig-example-dang-structural-1.pdf}
...
{\centering \includegraphics{C_sscan3_causal_interpretation/C__sscan3_causal_interpretation_files/figure-pdf/fig-example-swig-structural-1.pdf}
...
that encodes the relational part of the SWING.
...
{\centering \includegraphics{C_sscan3_causal_interpretation/C__sscan3_causal_interpretation_files/figure-pdf/fig-repeated-observations-dang-1.pdf}
...
{\centering \includegraphics{C_sscan3_causal_interpretation/C__sscan3_causal_interpretation_files/figure-pdf/fig-placeintervention-swing-1.pdf}
...
to other community districts through the exchange of individuals.
...
{\centering \includegraphics{C_sscan3_causal_interpretation/C__sscan3_causal_interpretation_files/figure-pdf/fig-edge-intervention-swing-1.pdf}
...
\hypertarget{estimands-2}{%
\subsection{Estimands}\label{estimands-2}}
...
{\centering \includegraphics{D_survey_instrument/../_const/images/image_app_graphic.png}
...
\hypertarget{sec-appendixquestionnaire}{%
\section{Study questionnaire}\label{sec-appendixquestionnaire}}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{targets}\SpecialCharTok{::}\FunctionTok{tar\_read}\NormalTok{(config\_list)[[}\DecValTok{1}\NormalTok{]][[}\StringTok{"variables"}\NormalTok{]] }\SpecialCharTok{|\textgreater{}}
\NormalTok{  purrr}\SpecialCharTok{::}\FunctionTok{map}\NormalTok{(}
    \ControlFlowTok{function}\NormalTok{(x) }\FunctionTok{return}\NormalTok{(}\FunctionTok{paste0}\NormalTok{(}
                              \StringTok{"::: \{.card\}}\SpecialCharTok{\textbackslash{}n\textbackslash{}n}\StringTok{"}\NormalTok{, }
\NormalTok{                              x[[}\StringTok{"label"}\NormalTok{]], }
                              \StringTok{"}\SpecialCharTok{\textbackslash{}n\textbackslash{}n}\StringTok{"}\NormalTok{,}
\NormalTok{                               x[[}\StringTok{"question"}\NormalTok{]],}
                              \StringTok{"}\SpecialCharTok{\textbackslash{}n\textbackslash{}n}\StringTok{"}\NormalTok{,}
                              \FunctionTok{paste}\NormalTok{(x[[}\StringTok{"response\_options"}\NormalTok{]], }\AttributeTok{collapse =} \StringTok{"; "}\NormalTok{),}
                              \StringTok{"}\SpecialCharTok{\textbackslash{}n\textbackslash{}n}\StringTok{:::"}
\NormalTok{                              ))}
\NormalTok{  ) }\SpecialCharTok{|\textgreater{}}
  \FunctionTok{paste}\NormalTok{(}\AttributeTok{collapse =} \StringTok{"}\SpecialCharTok{\textbackslash{}n\textbackslash{}n}\StringTok{"}\NormalTok{) }\SpecialCharTok{|\textgreater{}}
...
\textless18; 18-24; 25-34; 35-44; 45-54; 55+; 55+
...
How many queer and trans friends do you have who are important to you
for any reason? Count only those you have been in touch with over the
past 4 weeks.

Recent physical contacts

How many individuals have you had prolonged physical contact (but no
sexual contact) with in the past 4 weeks, excluding group settings?

Recent individual sex contacts

How many individuals have you had sex with in the past 4 weeks,
excluding group sex partners?
...
Over the past 4 weeks, have you had sex with two or more people at the
same time or had close physical contact with multiple people at the same
time? (like at a party, sport game, concert, show)
...
Cisgender Man; Non Binary; Transgender Man; Transgender Woman; Cisgender
Woman; Another Demographic; Another Demographic
...
Concert/Theatre/Show; Concert/Theatre/Show; Dark Room/Sex Party; Dark
Room/Sex Party; Dance Party; Sport Game; Private Residence; Something
Else
...
{\centering \includegraphics{D_survey_instrument/D__survey_instrument_files/figure-pdf/unnamed-chunk-3-1.pdf}
...
{\centering \includegraphics{E_marketing_comm/../_const/images/image_campaign_graphic.png}
...
{\centering \includegraphics{E_marketing_comm/../_const/images/image_creative_requirements.png}
...
{\centering \includegraphics{E_marketing_comm/E__marketing_comm_files/figure-pdf/unnamed-chunk-2-1.pdf}
...
But given that many public health data systems use measures for gender
...
{[}List in narrative form - reach out to martez for details on early
workshops{]}

Workshops with Martez and Jennifer AIDS Conference Abstracts Academic
talks WHO Talk

\hypertarget{sec-appendixresults}{%
\chapter{Supplementary results}\label{sec-appendixresults}}

\hypertarget{people}{%
\section{People}\label{people}}

\hypertarget{race-and-gender-1}{%
\subsection*{Race and gender}\label{race-and-gender-1}}
\addcontentsline{toc}{subsection}{Race and gender}

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{make\_table\_freq1}\NormalTok{(demo\_group) }\SpecialCharTok{|\textgreater{}}
  \FunctionTok{draw\_table\_1}\NormalTok{(}\StringTok{"Race{-}Gender"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
Black cisgender man & 131 & 10\% (10-11) \\ 
Cisgender woman & 62 & 5\% (4-6) \\ 
Latinx cisgender man & 198 & 15\% (14-17) \\ 
Non binary & 96 & 7\% (6-8) \\ 
Other cisgender man & 176 & 13\% (12-14) \\ 
Transgender man & 34 & 3\% (2-4) \\ 
Transgender woman & 29 & 2\% (2-2) \\ 
White cisgender man & 552 & 42\% (40-44) \\ 
\bottomrule
\end{longtable*}
...
\begin{Highlighting}[]
\FunctionTok{make\_table\_freq1}\NormalTok{(demo\_group) }\SpecialCharTok{|\textgreater{}}  
  \FunctionTok{plot\_bar}\NormalTok{() }\SpecialCharTok{+}
  \FunctionTok{theme\_mpxnyc\_bar}\NormalTok{() }\SpecialCharTok{+}
  \FunctionTok{scale\_axis\_mpxnyc}\NormalTok{() }\SpecialCharTok{+}
  \FunctionTok{scale\_fill\_mpxnyc}\NormalTok{(}\AttributeTok{name =} \StringTok{"Race / gender"}\NormalTok{) }
\end{Highlighting}
\end{Shaded}

\begin{figure}[H]

{\centering \includegraphics{G_supplementary_results/G__supplementary_results_files/figure-pdf/plot_bar_demogroup-1.pdf}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{make\_table\_freq1}\NormalTok{(age) }\SpecialCharTok{|\textgreater{}}
  \FunctionTok{draw\_table\_1}\NormalTok{(}\StringTok{"Age"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
  \FunctionTok{make\_table\_freq1}\NormalTok{(age) }\SpecialCharTok{|\textgreater{}}
    ...
    \FunctionTok{theme\_mpxnyc\_bar}\NormalTok{() }\SpecialCharTok{+}
    \FunctionTok{scale\_axis\_mpxnyc}\NormalTok{() }\SpecialCharTok{+}
    \FunctionTok{scale\_fill\_mpxnyc}\NormalTok{(}\AttributeTok{name =} \StringTok{"Age group"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{figure}[H]

{\centering \includegraphics{G_supplementary_results/G__supplementary_results_files/figure-pdf/plot_bar_age-1.pdf}

}

\end{figure}

\hypertarget{recruitment-channel}{%
\subsection*{Recruitment Channel}\label{recruitment-channel}}
\addcontentsline{toc}{subsection}{Recruitment Channel}

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{make\_table\_freq1}\NormalTok{(channel) }\SpecialCharTok{|\textgreater{}}
  \FunctionTok{draw\_table\_1}\NormalTok{(}\StringTok{"Channel"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\midrule\addlinespace[2.5pt]
Grindr & 706 & 54\% (52-55) \\ 
Instagram & 112 & 9\% (8-10) \\ 
Partner toolkit & 142 & 11\% (10-12) \\ 
Twitter & 62 & 5\% (4-5) \\ 
Unknown & 282 & 22\% (20-24) \\ 
\bottomrule
\end{longtable*}
...
\FunctionTok{make\_table\_freq1}\NormalTok{(channel) }\SpecialCharTok{|\textgreater{}}
  ...
  \FunctionTok{theme\_mpxnyc\_bar}\NormalTok{() }\SpecialCharTok{+}
  \FunctionTok{scale\_axis\_mpxnyc}\NormalTok{() }\SpecialCharTok{+}
  \FunctionTok{scale\_fill\_mpxnyc}\NormalTok{(}\AttributeTok{name =} \StringTok{"Recruitment}\SpecialCharTok{\textbackslash{}n}\StringTok{channel"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{figure}[H]

{\centering \includegraphics{G_supplementary_results/G__supplementary_results_files/figure-pdf/plot_bar_channel-1.pdf}
...
\addcontentsline{toc}{subsection}{Gender identity}

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{make\_table\_freq1}\NormalTok{(genderId) }\SpecialCharTok{|\textgreater{}}
  \FunctionTok{draw\_table\_1}\NormalTok{(}\StringTok{"Gender"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\FunctionTok{make\_table\_freq1}\NormalTok{(genderId) }\SpecialCharTok{|\textgreater{}}  
  \FunctionTok{plot\_bar}\NormalTok{() }\SpecialCharTok{+}
  \FunctionTok{theme\_mpxnyc\_bar}\NormalTok{() }\SpecialCharTok{+}
  \FunctionTok{scale\_axis\_mpxnyc}\NormalTok{(}\StringTok{"MPX NYC sample proportion"}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{scale\_fill\_mpxnyc}\NormalTok{(}\AttributeTok{name =} \StringTok{"Gender}\SpecialCharTok{\textbackslash{}n}\StringTok{modality"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{figure}[H]

{\centering \includegraphics{G_supplementary_results/G__supplementary_results_files/figure-pdf/plot_bar_gender-1.pdf}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{make\_table\_freq1}\NormalTok{(race) }\SpecialCharTok{|\textgreater{}}
  \FunctionTok{draw\_table\_1}\NormalTok{(}\StringTok{"Race"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\toprule
 & N & \% (CI) \\ 
\midrule\addlinespace[2.5pt]
Another Group & 57 & 4\% (4-5) \\ 
Asian & 84 & 6\% (5-7) \\ 
Black & 156 & 12\% (11-13) \\ 
Latinx & 231 & 18\% (16-20) \\ 
Multi Racial & 79 & 6\% (6-7) \\ 
White & 695 & 53\% (51-55) \\ 
\bottomrule
\end{longtable*}
...
\FunctionTok{make\_table\_freq1}\NormalTok{(race) }\SpecialCharTok{|\textgreater{}}
  ...
  \FunctionTok{theme\_mpxnyc\_bar}\NormalTok{() }\SpecialCharTok{+}
  \FunctionTok{scale\_axis\_mpxnyc}\NormalTok{() }\SpecialCharTok{+}
  \FunctionTok{scale\_fill\_mpxnyc}\NormalTok{(}\AttributeTok{name =} \StringTok{"Race"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{figure}[H]

{\centering \includegraphics{G_supplementary_results/G__supplementary_results_files/figure-pdf/plot_bar_race-1.pdf}

}

\end{figure}

\hypertarget{sexual-orientation-1}{%
\subsection*{Sexual Orientation}\label{sexual-orientation-1}}
\addcontentsline{toc}{subsection}{Sexual Orientation}

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{make\_table\_freq1}\NormalTok{(sexOrientation) }\SpecialCharTok{|\textgreater{}}
  \FunctionTok{draw\_table\_1}\NormalTok{(}\StringTok{"Sexual Orientation"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{longtable*}{lcc}
\caption*{
{\large Sexual Orientation} \\ 
{\small (MPX NYC, 2022)}
} \\ 
\toprule
 & N & \% (CI) \\ 
\midrule\addlinespace[2.5pt]
Bisexual & 186 & 14\% (13-15) \\ 
Gay & 860 & 66\% (63-67) \\ 
Queer & 159 & 12\% (11-14) \\ 
...
\FunctionTok{make\_table\_freq1}\NormalTok{(sexOrientation) }\SpecialCharTok{|\textgreater{}}    
  ...
  \FunctionTok{theme\_mpxnyc\_bar}\NormalTok{() }\SpecialCharTok{+}
  \FunctionTok{scale\_axis\_mpxnyc}\NormalTok{() }\SpecialCharTok{+}
  \FunctionTok{scale\_fill\_mpxnyc}\NormalTok{(}\AttributeTok{name =} \StringTok{"Sexual}\SpecialCharTok{\textbackslash{}n}\StringTok{orientation"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{figure}[H]

{\centering \includegraphics{G_supplementary_results/G__supplementary_results_files/figure-pdf/plot_bar_sexorientation-1.pdf}

}

\end{figure}

\hypertarget{sexual-or-physical-contact-with-more-than-two-other-people-at-the-same-time}{%
\subsection*{Sexual or physical contact with more than two other people
at the same
time}\label{sexual-or-physical-contact-with-more-than-two-other-people-at-the-same-time}}
\addcontentsline{toc}{subsection}{Sexual or physical contact with more
than two other people at the same time}

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{make\_table\_freq1}\NormalTok{(groupSex) }\SpecialCharTok{|\textgreater{}} 
  \FunctionTok{draw\_table\_1}\NormalTok{(}\StringTok{"Group Sex / Contact past 4 weeks"}\NormalTok{) }
\end{Highlighting}
\end{Shaded}

\begin{longtable*}{lcc}
\caption*{
{\large Group Sex / Contact past 4 weeks} \\ 
{\small (MPX NYC, 2022)}
} \\ 
\toprule
 & N & \% (CI) \\ 
...
\FunctionTok{make\_table\_freq1}\NormalTok{(groupSex) }\SpecialCharTok{|\textgreater{}}  
  ...
  \FunctionTok{theme\_mpxnyc\_bar}\NormalTok{() }\SpecialCharTok{+}
  \FunctionTok{scale\_axis\_mpxnyc}\NormalTok{() }\SpecialCharTok{+}
  \FunctionTok{scale\_fill\_mpxnyc}\NormalTok{(}\AttributeTok{name =} \StringTok{"Group contact"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{figure}[H]

{\centering \includegraphics{G_supplementary_results/G__supplementary_results_files/figure-pdf/plot_bar_groupsex-1.pdf}

}

\end{figure}

\hypertarget{sexual-orientation-by-race-and-gender}{%
\subsection*{Sexual orientation by race and
gender}\label{sexual-orientation-by-race-and-gender}}
\addcontentsline{toc}{subsection}{Sexual orientation by race and gender}

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{make\_table\_freq2}\NormalTok{(sexOrientation, demo\_group)  }\SpecialCharTok{|\textgreater{}}
  \FunctionTok{draw\_table\_2}\NormalTok{(}\StringTok{"Sexual Orientation by Race{-}Gender"}\NormalTok{) }
\end{Highlighting}
\end{Shaded}

\begin{longtable*}{lcc}
\caption*{
{\large Sexual Orientation by Race-Gender} \\ 
{\small (MPX NYC, 2022)}
} \\ 
\toprule
 & N & \% (CI) \\ 
\midrule\addlinespace[2.5pt]
\multicolumn{3}{l}{Another demographic} \\ 
\midrule\addlinespace[2.5pt]
Bisexual & 3 & 12\% (3-22) \\ 
Gay & 5 & 19\% (13-25) \\ 
Queer & 11 & 42\% (29-60) \\ 
Something Else & 6 & 23\% (15-36) \\ 
Straight & 1 & 4\% (3-9) \\ 
\midrule\addlinespace[2.5pt]
\multicolumn{3}{l}{Black cisgender man} \\ 
\midrule\addlinespace[2.5pt]
Bisexual & 24 & 18\% (14-26) \\ 
Gay & 88 & 67\% (58-73) \\ 
Queer & 9 & 7\% (5-10) \\ 
Something Else & 4 & 3\% (2-5) \\ 
Straight & 6 & 5\% (2-5) \\ 
\midrule\addlinespace[2.5pt]
\multicolumn{3}{l}{Cisgender woman} \\ 
\midrule\addlinespace[2.5pt]
Bisexual & 19 & 31\% (18-41) \\ 
Gay & 4 & 6\% (5-12) \\ 
Queer & 16 & 26\% (23-33) \\ 
Straight & 23 & 37\% (29-47) \\ 
\midrule\addlinespace[2.5pt]
\multicolumn{3}{l}{Latinx cisgender man} \\ 
\midrule\addlinespace[2.5pt]
Bisexual & 24 & 12\% (11-15) \\ 
Gay & 158 & 80\% (77-81) \\ 
Queer & 9 & 5\% (4-6) \\ 
Something Else & 2 & 1\% (1-2) \\ 
Straight & 5 & 3\% (1-4) \\ 
\midrule\addlinespace[2.5pt]
\multicolumn{3}{l}{Non binary} \\ 
\midrule\addlinespace[2.5pt]
Bisexual & 18 & 19\% (14-25) \\ 
Gay & 24 & 25\% (20-30) \\ 
Queer & 53 & 55\% (46-63) \\ 
Something Else & 1 & 1\% (1-3) \\ 
\midrule\addlinespace[2.5pt]
\multicolumn{3}{l}{Other cisgender man} \\ 
\midrule\addlinespace[2.5pt]
Bisexual & 24 & 14\% (11-18) \\ 
Gay & 136 & 77\% (74-78) \\ 
Queer & 10 & 6\% (4-8) \\ 
Straight & 6 & 3\% (2-6) \\ 
\midrule\addlinespace[2.5pt]
\multicolumn{3}{l}{Transgender man} \\ 
\midrule\addlinespace[2.5pt]
Bisexual & 10 & 29\% (16-44) \\ 
Gay & 7 & 21\% (9-35) \\ 
Queer & 14 & 41\% (23-68) \\ 
Something Else & 1 & 3\% (2-6) \\ 
Straight & 2 & 6\% (3-14) \\ 
\midrule\addlinespace[2.5pt]
\multicolumn{3}{l}{Transgender woman} \\ 
\midrule\addlinespace[2.5pt]
Bisexual & 7 & 24\% (14-31) \\ 
Gay & 3 & 10\% (11-15) \\ 
Queer & 4 & 14\% (8-28) \\ 
Something Else & 6 & 21\% (8-28) \\ 
Straight & 9 & 31\% (25-41) \\ 
\midrule\addlinespace[2.5pt]
\multicolumn{3}{l}{White cisgender man} \\ 
\midrule\addlinespace[2.5pt]
Bisexual & 57 & 10\% (8-12) \\ 
Gay & 435 & 79\% (76-81) \\ 
Queer & 33 & 6\% (5-7) \\ 
Something Else & 15 & 3\% (2-4) \\ 
Straight & 12 & 2\% (2-3) \\ 
\bottomrule
\end{longtable*}
...
{\centering \includegraphics{G_supplementary_results/G__supplementary_results_files/figure-pdf/plot_radar_grid_sexOrientation_demo_group-1.pdf}

}

\end{figure}

\hypertarget{mpox-vaccination-status-by-race-and-gender}{%
\subsection*{MPOX vaccination status by race and
gender}\label{mpox-vaccination-status-by-race-and-gender}}
\addcontentsline{toc}{subsection}{MPOX vaccination status by race and
gender}

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{make\_table\_freq2}\NormalTok{(monkeypoxVaccine, demo\_group) }\SpecialCharTok{|\textgreater{}}
  \FunctionTok{draw\_table\_2}\NormalTok{( }\StringTok{"Mpox Vax Status by Race{-}Gender"}\NormalTok{) }
\end{Highlighting}
\end{Shaded}

\begin{longtable*}{lcc}
\caption*{
{\large Mpox Vax Status by Race-Gender} \\ 
{\small (MPX NYC, 2022)}
} \\ 
\toprule
 & N & \% (CI) \\ 
...
{\centering \includegraphics{G_supplementary_results/G__supplementary_results_files/figure-pdf/plot_radar_grid_monkeypoxVaccine_demo_group-1.pdf}

}

\end{figure}

\hypertarget{hiv-and-prep-status-by-race-and-gender}{%
\subsection*{HIV and PrEP status by race and
gender}\label{hiv-and-prep-status-by-race-and-gender}}
\addcontentsline{toc}{subsection}{HIV and PrEP status by race and
gender}

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{make\_table\_freq2}\NormalTok{(hivPrep, demo\_group) }\SpecialCharTok{|\textgreater{}}
  \FunctionTok{draw\_table\_2}\NormalTok{(}\StringTok{"HIV and PrEP Status by Race{-}Gender"}\NormalTok{) }
\end{Highlighting}
\end{Shaded}

\begin{longtable*}{lcc}
\caption*{
{\large HIV and PrEP Status by Race-Gender} \\ 
{\small (MPX NYC, 2022)}
} \\ 
\toprule
 & N & \% (CI) \\ 
...
{\centering \includegraphics{G_supplementary_results/G__supplementary_results_files/figure-pdf/plot_radar_grid_hivPrep_demo_group-1.pdf}
...
\hypertarget{type-of-venue-for-contact-vs.-type-of-contact}{%
\subsection*{Type of venue for contact vs.~type of
contact}\label{type-of-venue-for-contact-vs.-type-of-contact}}
\addcontentsline{toc}{subsection}{Type of venue for contact vs.~type of
contact}

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{make\_table\_freq2}\NormalTok{(placeType, placeSex, }\AttributeTok{person\_analysis =} \ConstantTok{FALSE}\NormalTok{) }\SpecialCharTok{|\textgreater{}}
  \FunctionTok{draw\_table\_2}\NormalTok{(}\StringTok{"Venue type vs. sexual contact"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{longtable*}{lcc}
\caption*{
{\large Venue type vs. sexual contact} \\ 
{\small (MPX NYC, 2022)}
} \\ 
\toprule
 & N & \% (CI) \\ 
\midrule\addlinespace[2.5pt]
\multicolumn{3}{l}{Did not have sex} \\ 
\midrule\addlinespace[2.5pt]
Concert/Theatre/Show & 63 & 24\% (20-28) \\ 
Dance Party & 110 & 42\% (35-49) \\ 
Dark Room/Sex Party & 10 & 4\% (3-6) \\ 
Private Residence & 28 & 11\% (10-15) \\ 
Something Else & 40 & 15\% (10-20) \\ 
Sport Game & 13 & 5\% (3-7) \\ 
\midrule\addlinespace[2.5pt]
\multicolumn{3}{l}{Had sex} \\ 
\midrule\addlinespace[2.5pt]
Concert/Theatre/Show & 4 & 1\% (1-2) \\ 
Dance Party & 33 & 9\% (8-12) \\ 
Dark Room/Sex Party & 52 & 15\% (11-17) \\ 
Private Residence & 222 & 63\% (61-65) \\ 
Something Else & 40 & 11\% (10-15) \\ 
Sport Game & 1 & 0\% (0-1) \\ 
\bottomrule
\end{longtable*}
...
\begin{Highlighting}[]
\FunctionTok{make\_table\_freq2}\NormalTok{(placeType, placeSex, }\AttributeTok{person\_analysis =} \ConstantTok{FALSE}\NormalTok{) }\SpecialCharTok{|\textgreater{}}
  \FunctionTok{plot\_radar\_stratified}\NormalTok{() }\SpecialCharTok{+}
  \FunctionTok{scale\_radial\_mpxnyc}\NormalTok{() }\SpecialCharTok{+}
  \FunctionTok{scale\_fill\_mpxnyc}\NormalTok{(}\AttributeTok{name =} \StringTok{""}\NormalTok{) }\SpecialCharTok{+}
...
{\centering \includegraphics{G_supplementary_results/G__supplementary_results_files/figure-pdf/plot_radar_facet_placetype_placesex-1.pdf}

}

\end{figure}

\hypertarget{type-of-venue-for-contact-vs.-distance-from-home}{%
\subsection*{Type of venue for contact vs.~distance from
home}\label{type-of-venue-for-contact-vs.-distance-from-home}}
\addcontentsline{toc}{subsection}{Type of venue for contact vs.~distance
from home}

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{make\_table\_freq2}\NormalTok{(placeType, distanceFromHome, }\AttributeTok{person\_analysis =} \ConstantTok{FALSE}\NormalTok{) }\SpecialCharTok{|\textgreater{}}
  \FunctionTok{draw\_table\_2}\NormalTok{(}\StringTok{"Venue type vs. distance from home"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{longtable*}{lcc}
\caption*{
{\large Venue type vs. distance from home} \\ 
{\small (MPX NYC, 2022)}
} \\ 
\toprule
 & N & \% (CI) \\ 
\midrule\addlinespace[2.5pt]
\multicolumn{3}{l}{Different Borough} \\ 
\midrule\addlinespace[2.5pt]
Concert/Theatre/Show & 26 & 13\% (10-15) \\ 
Dance Party & 64 & 32\% (28-37) \\ 
Dark Room/Sex Party & 29 & 14\% (12-21) \\ 
Private Residence & 47 & 23\% (16-28) \\ 
Something Else & 25 & 12\% (10-15) \\ 
Sport Game & 9 & 4\% (3-7) \\ 
\midrule\addlinespace[2.5pt]
\multicolumn{3}{l}{Same Borough} \\ 
\midrule\addlinespace[2.5pt]
Concert/Theatre/Show & 36 & 16\% (11-20) \\ 
Dance Party & 56 & 24\% (21-31) \\ 
Dark Room/Sex Party & 22 & 10\% (6-9) \\ 
Private Residence & 73 & 32\% (30-38) \\ 
Something Else & 39 & 17\% (13-20) \\ 
Sport Game & 4 & 2\% (1-3) \\ 
\midrule\addlinespace[2.5pt]
\multicolumn{3}{l}{Same Community District} \\ 
\midrule\addlinespace[2.5pt]
Concert/Theatre/Show & 5 & 3\% (1-5) \\ 
Dance Party & 23 & 12\% (8-17) \\ 
Dark Room/Sex Party & 11 & 6\% (4-7) \\ 
Private Residence & 130 & 70\% (68-76) \\ 
Something Else & 16 & 9\% (6-10) \\ 
Sport Game & 1 & 1\% (1-1) \\ 
\bottomrule
\end{longtable*}
...
{\centering \includegraphics{G_supplementary_results/G__supplementary_results_files/figure-pdf/plot_radar_facet_placetype_distancefromhome-1.pdf}

}

\end{figure}

\hypertarget{place-type-vs.-distance-from-home-vs.-sex-contact}{%
\subsection*{Place type vs.~Distance from home vs.~Sex
contact}\label{place-type-vs.-distance-from-home-vs.-sex-contact}}
\addcontentsline{toc}{subsection}{Place type vs.~Distance from home
vs.~Sex contact}

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{make\_table\_freq3}\NormalTok{( placeType, distanceFromHome, placeSex, }\AttributeTok{person\_analysis =} \ConstantTok{FALSE}\NormalTok{)  }\SpecialCharTok{|\textgreater{}}
  \FunctionTok{draw\_table\_3}\NormalTok{(}\StringTok{"Venue type by home distance and sexual contact"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{longtable*}{lcc}
\caption*{
{\large Venue type by home distance and sexual contact} \\ 
{\small (MPX NYC, 2022)}
} \\ 
\toprule
 & N & \% (CI) \\ 
\midrule\addlinespace[2.5pt]
\multicolumn{3}{l}{Different Borough - Did not have sex} \\ 
\midrule\addlinespace[2.5pt]
Concert/Theatre/Show & 23 & 22\% (18-22) \\ 
Dance Party & 50 & 47\% (41-52) \\ 
Dark Room/Sex Party & 4 & 4\% (4-10) \\ 
Private Residence & 8 & 8\% (5-12) \\ 
Something Else & 13 & 12\% (8-15) \\ 
Sport Game & 8 & 8\% (6-13) \\ 
\midrule\addlinespace[2.5pt]
\multicolumn{3}{l}{Different Borough - Had sex} \\ 
\midrule\addlinespace[2.5pt]
Concert/Theatre/Show & 3 & 3\% (1-6) \\ 
Dance Party & 14 & 15\% (12-21) \\ 
Dark Room/Sex Party & 25 & 27\% (20-36) \\ 
Private Residence & 39 & 41\% (29-45) \\ 
Something Else & 12 & 13\% (9-21) \\ 
Sport Game & 1 & 1\% (1-3) \\ 
\midrule\addlinespace[2.5pt]
\multicolumn{3}{l}{Same Borough - Did not have sex} \\ 
\midrule\addlinespace[2.5pt]
Concert/Theatre/Show & 35 & 30\% (20-36) \\ 
Dance Party & 43 & 37\% (29-47) \\ 
Dark Room/Sex Party & 3 & 3\% (1-4) \\ 
Private Residence & 10 & 9\% (7-14) \\ 
Something Else & 21 & 18\% (11-25) \\ 
Sport Game & 4 & 3\% (2-5) \\ 
\midrule\addlinespace[2.5pt]
\multicolumn{3}{l}{Same Borough - Had sex} \\ 
\midrule\addlinespace[2.5pt]
Concert/Theatre/Show & 1 & 1\% (1-3) \\ 
Dance Party & 13 & 11\% (8-16) \\ 
Dark Room/Sex Party & 19 & 17\% (9-15) \\ 
Private Residence & 63 & 55\% (53-61) \\ 
Something Else & 18 & 16\% (13-18) \\ 
\midrule\addlinespace[2.5pt]
\multicolumn{3}{l}{Same Community District - Did not have sex} \\ 
\midrule\addlinespace[2.5pt]
Concert/Theatre/Show & 5 & 12\% (6-24) \\ 
Dance Party & 17 & 40\% (27-57) \\ 
Dark Room/Sex Party & 3 & 7\% (2-12) \\ 
Private Residence & 10 & 24\% (19-40) \\ 
Something Else & 6 & 14\% (7-17) \\ 
Sport Game & 1 & 2\% (2-3) \\ 
\midrule\addlinespace[2.5pt]
\multicolumn{3}{l}{Same Community District - Had sex} \\ 
\midrule\addlinespace[2.5pt]
Dance Party & 6 & 4\% (2-5) \\ 
Dark Room/Sex Party & 8 & 6\% (4-8) \\ 
Private Residence & 120 & 83\% (82-87) \\ 
Something Else & 10 & 7\% (4-11) \\ 
\bottomrule
\end{longtable*}
...
{\centering \includegraphics{G_supplementary_results/G__supplementary_results_files/figure-pdf/plot_radar_facet_placetype_placesex_distancefromhome-1.pdf}

}

\end{figure}

\hypertarget{spatial-mixing}{%
\section{Spatial mixing}\label{spatial-mixing}}

\hypertarget{spatial-mixing-coefficient-by-age}{%
\subsection*{Spatial mixing coefficient by
age}\label{spatial-mixing-coefficient-by-age}}
\addcontentsline{toc}{subsection}{Spatial mixing coefficient by age}

\subsubsection{Show plot}
...
\FunctionTok{make\_plotdata\_age\_mixing\_matrix}\NormalTok{() }\SpecialCharTok{|\textgreater{}}
  ...
  \FunctionTok{scale\_fill\_mpxnyc\_gradient}\NormalTok{() }\SpecialCharTok{+}
  \FunctionTok{scale\_color\_mpxnyc\_gradient}\NormalTok{() }\SpecialCharTok{+}
  \FunctionTok{theme\_mpxnyc\_mixing}\NormalTok{(}\AttributeTok{plot.margin =}\NormalTok{ ggplot2}\SpecialCharTok{::}\FunctionTok{unit}\NormalTok{(}\DecValTok{10}\SpecialCharTok{*}\FunctionTok{c}\NormalTok{(}\DecValTok{10}\NormalTok{,}\DecValTok{10}\NormalTok{,}\DecValTok{10}\NormalTok{,}\DecValTok{10}\NormalTok{), }\StringTok{"pt"}\NormalTok{))}
\end{Highlighting}
\end{Shaded}

\begin{figure}[H]

{\centering \includegraphics{G_supplementary_results/G__supplementary_results_files/figure-pdf/fig-age-mixing-matrix-app-1.pdf}

}

\caption{\label{fig-age-mixing-matrix-app}Social mixing matrix for MPX
NYC participants by age}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{make\_table\_mixing\_2}\NormalTok{(age) }\SpecialCharTok{|\textgreater{}}
 \FunctionTok{draw\_table\_matrix\_mixing}\NormalTok{(}\StringTok{"Age"}\NormalTok{)  }
\end{Highlighting}
\end{Shaded}
...
\midrule\addlinespace[2.5pt]
18-24 & 18-24 & 37\% (25 to 48)  \\ 
18-24 & 25-34 & -4\% (-6 to 1)  \\ 
18-24 & 35-44 & -3\% (-5 to -2)  \\ 
18-24 & 45-54 & -11\% (-18 to -4)  \\ 
18-24 & 55+ & -27\% (-35 to -19)  \\ 
25-34 & 18-24 & -3\% (-6 to 2)  \\ 
25-34 & 25-34 & 11\% (9 to 15)  \\ 
25-34 & 35-44 & -3\% (-6 to -1)  \\ 
25-34 & 45-54 & -12\% (-15 to -6)  \\ 
25-34 & 55+ & -26\% (-34 to -20)  \\ 
35-44 & 18-24 & -3\% (-7 to 4)  \\ 
35-44 & 25-34 & -1\% (-5 to 2)  \\ 
35-44 & 35-44 & 15\% (9 to 19)  \\ 
35-44 & 45-54 & -12\% (-15 to -7)  \\ 
35-44 & 55+ & -20\% (-27 to -11)  \\ 
45-54 & 18-24 & 3\% (-7 to 11)  \\ 
45-54 & 25-34 & -3\% (-6 to 4)  \\ 
45-54 & 35-44 & -5\% (-10 to -1)  \\ 
45-54 & 45-54 & 30\% (16 to 44)  \\ 
...
\hypertarget{spatial-mixing-coefficient-by-race-gender}{%
\subsection*{Spatial mixing coefficient by
race-Gender}\label{spatial-mixing-coefficient-by-race-gender}}
\addcontentsline{toc}{subsection}{Spatial mixing coefficient by
race-Gender}
...
{\centering \includegraphics{G_supplementary_results/G__supplementary_results_files/figure-pdf/fig-racegender-mixing-matrix-app-1.pdf}

}

\caption{\label{fig-racegender-mixing-matrix-app}Social mixing matrices
for MPX NYC participants by race-gender}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{make\_table\_mixing\_2}\NormalTok{(demo\_group) }\SpecialCharTok{|\textgreater{}}
 \FunctionTok{draw\_table\_matrix\_mixing}\NormalTok{(}\StringTok{"Race{-}gender"}\NormalTok{)  }
\end{Highlighting}
\end{Shaded}
...
White cisgender man & Black cisgender man & -24\% (-29 to -18)  \\ 
White cisgender man & Transgender man & 7\% (-6 to 16)  \\ 
White cisgender man & Transgender woman & -19\% (-36 to -6)  \\ 
White cisgender man & Non binary & -4\% (-10 to 4)  \\ 
White cisgender man & Cisgender woman & -14\% (-20 to -4)  \\ 
White cisgender man & Other cisgender man & -2\% (-5 to 3)  \\ 
White cisgender man & Another demographic & 29\% (-15 to 69)  \\ 
Latinx cisgender man & White cisgender man & -16\% (-22 to -11)  \\ 
Latinx cisgender man & Latinx cisgender man & 45\% (39 to 63)  \\ 
Latinx cisgender man & Black cisgender man & 2\% (-5 to 13)  \\ 
Latinx cisgender man & Transgender man & 14\% (2 to 37)  \\ 
Latinx cisgender man & Transgender woman & -22\% (-43 to -2)  \\ 
Latinx cisgender man & Non binary & -4\% (-14 to 3)  \\ 
Latinx cisgender man & Cisgender woman & -13\% (-28 to -1)  \\ 
Latinx cisgender man & Other cisgender man & -2\% (-12 to 2)  \\ 
Latinx cisgender man & Another demographic & 31\% (0 to 74)  \\ 
Black cisgender man & White cisgender man & -25\% (-28 to -22)  \\ 
Black cisgender man & Latinx cisgender man & 10\% (-8 to 19)  \\ 
Black cisgender man & Black cisgender man & 95\% (64 to 143)  \\ 
Black cisgender man & Transgender man & -25\% (-44 to -1)  \\ 
Black cisgender man & Transgender woman & 1\% (-22 to 41)  \\ 
Black cisgender man & Non binary & -12\% (-31 to 3)  \\ 
Black cisgender man & Cisgender woman & 11\% (-19 to 29)  \\ 
Black cisgender man & Other cisgender man & -5\% (-16 to 4)  \\ 
Black cisgender man & Another demographic & 33\% (18 to 63)  \\ 
Transgender man & White cisgender man & -11\% (-19 to 0)  \\ 
Transgender man & Latinx cisgender man & 0\% (-14 to 12)  \\ 
Transgender man & Black cisgender man & -20\% (-40 to 11)  \\ 
Transgender man & Transgender man & 215\% (72 to 363)  \\ 
Transgender man & Transgender woman & -8\% (-32 to 33)  \\ 
Transgender man & Non binary & 61\% (26 to 129)  \\ 
Transgender man & Cisgender woman & -30\% (-62 to -9)  \\ 
Transgender man & Other cisgender man & -14\% (-28 to 6)  \\ 
Transgender man & Another demographic & 13\% (-37 to 49)  \\ 
Transgender woman & White cisgender man & -13\% (-26 to 1)  \\ 
Transgender woman & Latinx cisgender man & -12\% (-30 to 17)  \\ 
Transgender woman & Black cisgender man & 12\% (-17 to 56)  \\ 
Transgender woman & Transgender man & -5\% (-34 to 39)  \\ 
Transgender woman & Transgender woman & 131\% (-30 to 332)  \\ 
Transgender woman & Non binary & 22\% (1 to 48)  \\ 
Transgender woman & Cisgender woman & -5\% (-35 to 59)  \\ 
Transgender woman & Other cisgender man & 9\% (-24 to 31)  \\ 
Transgender woman & Another demographic & 59\% (-44 to 121)  \\ 
Non binary & White cisgender man & -9\% (-14 to -1)  \\ 
Non binary & Latinx cisgender man & -8\% (-21 to 0)  \\ 
Non binary & Black cisgender man & -16\% (-30 to -1)  \\ 
Non binary & Transgender man & 56\% (21 to 103)  \\ 
Non binary & Transgender woman & 12\% (-12 to 38)  \\ 
Non binary & Non binary & 66\% (35 to 106)  \\ 
Non binary & Cisgender woman & -19\% (-28 to -4)  \\ 
Non binary & Other cisgender man & 5\% (-7 to 19)  \\ 
Non binary & Another demographic & 22\% (-20 to 69)  \\ 
Cisgender woman & White cisgender man & -5\% (-10 to -1)  \\ 
Cisgender woman & Latinx cisgender man & 3\% (-8 to 15)  \\ 
Cisgender woman & Black cisgender man & 3\% (-8 to 22)  \\ 
Cisgender woman & Transgender man & -15\% (-51 to 12)  \\ 
Cisgender woman & Transgender woman & -19\% (-50 to 51)  \\ 
Cisgender woman & Non binary & -13\% (-22 to 9)  \\ 
Cisgender woman & Cisgender woman & 85\% (45 to 117)  \\ 
Cisgender woman & Other cisgender man & -17\% (-26 to -3)  \\ 
Cisgender woman & Another demographic & 52\% (-13 to 94)  \\ 
Other cisgender man & White cisgender man & -6\% (-11 to 1)  \\ 
Other cisgender man & Latinx cisgender man & -4\% (-10 to 0)  \\ 
Other cisgender man & Black cisgender man & -5\% (-9 to 0)  \\ 
Other cisgender man & Transgender man & 10\% (-8 to 27)  \\ 
Other cisgender man & Transgender woman & -13\% (-35 to 8)  \\ 
Other cisgender man & Non binary & 0\% (-10 to 10)  \\ 
Other cisgender man & Cisgender woman & -23\% (-29 to -12)  \\ 
Other cisgender man & Other cisgender man & 32\% (20 to 43)  \\ 
Other cisgender man & Another demographic & 20\% (-11 to 47)  \\ 
Another demographic & White cisgender man & 0\% (-22 to 17)  \\ 
Another demographic & Latinx cisgender man & -6\% (-18 to 8)  \\ 
Another demographic & Black cisgender man & -6\% (-35 to 20)  \\ 
Another demographic & Transgender man & -3\% (-36 to 17)  \\ 
Another demographic & Transgender woman & -25\% (-55 to 5)  \\ 
Another demographic & Non binary & -13\% (-43 to 15)  \\ 
Another demographic & Cisgender woman & -14\% (-45 to 8)  \\ 
Another demographic & Other cisgender man & -1\% (-21 to 21)  \\ 
Another demographic & Another demographic & 175\% (33 to 511)  \\ 
\bottomrule
\end{longtable*}

\hypertarget{spatial-mixing-coefficient-by-sexual-orientation}{%
\subsection*{Spatial mixing coefficient by sexual
orientation}\label{spatial-mixing-coefficient-by-sexual-orientation}}
\addcontentsline{toc}{subsection}{Spatial mixing coefficient by sexual
orientation}
...
{\centering \includegraphics{G_supplementary_results/G__supplementary_results_files/figure-pdf/fig-sexorientation-mixing-matrix-app-1.pdf}

}

\caption{\label{fig-sexorientation-mixing-matrix-app}Social mixing
matrix for MPX NYC participants by sexual orientation}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{make\_table\_mixing\_2}\NormalTok{(sexOrientation) }\SpecialCharTok{|\textgreater{}}
 \FunctionTok{draw\_table\_matrix\_mixing}\NormalTok{(}\StringTok{"Sexual orientation"}\NormalTok{)  }
\end{Highlighting}
\end{Shaded}
...
Gay & Straight & -18\% (-24 to -9)  \\ 
Gay & Queer & 5\% (-1 to 10)  \\ 
Gay & Something Else & -20\% (-31 to -13)  \\ 
Bisexual & Gay & -5\% (-11 to -2)  \\ 
Bisexual & Bisexual & 21\% (14 to 35)  \\ 
Bisexual & Straight & 5\% (-7 to 23)  \\ 
Bisexual & Queer & 4\% (-5 to 13)  \\ 
Bisexual & Something Else & -7\% (-24 to 7)  \\ 
Straight & Gay & -8\% (-14 to -4)  \\ 
Straight & Bisexual & 3\% (-8 to 20)  \\ 
Straight & Straight & 119\% (59 to 187)  \\ 
Straight & Queer & -5\% (-16 to 1)  \\ 
Straight & Something Else & -18\% (-39 to 8)  \\ 
Queer & Gay & -2\% (-5 to 0)  \\ 
Queer & Bisexual & -10\% (-15 to -2)  \\ 
Queer & Straight & -19\% (-29 to -11)  \\ 
Queer & Queer & 37\% (29 to 45)  \\ 
Queer & Something Else & -38\% (-49 to -32)  \\ 
Something Else & Gay & -12\% (-22 to -2)  \\ 
Something Else & Bisexual & 9\% (-12 to 27)  \\ 
Something Else & Straight & -15\% (-34 to 10)  \\ 
Something Else & Queer & -12\% (-24 to 0)  \\ 
Something Else & Something Else & 294\% (154 to 528)  \\ 
\bottomrule
\end{longtable*}

\hypertarget{responding-to-an-oubreak}{%
\chapter{*Responding to an oubreak}\label{responding-to-an-oubreak}}

Draft Project Proposal:

An Operational Guidance for Community-Led Emergency Response to Health
Emergencies
...
\hypertarget{outline-of-operational-guidance}{%
\section{Outline of Operational
Guidance}\label{outline-of-operational-guidance}}

\hfill\break

The main content of the document will be divided into four chapters,
which each discuss one of four core functions of the plan: Coordination,
Surveillance, Health Communications, and Resourcing. These will be
bookended by a background chapter and a chapter outlining an operational
structure for organizing the work outlined in the core chapters.~
...
\item
  ...
  \begin{itemize}
  \item
    To collect and disseminate information about the health emergency
    among stakeholders
  \item
    To collect and disseminate information about the experiences of
    community-based organizations and NYC government agencies responding
    to the health emergency
  \item
    To coordinate programmatic activities by NYC agencies and civil
...
\item
  ...
  \begin{itemize}
  \item
    Current NYC mechanisms for coordinating action in emergencies as
    well as an assessment of their strengths and weaknesses
  \item
    Current civil society mechanisms for coordinating action in
    emergencies as well as an assessment of their strengths and
    weaknesses
  \end{itemize}
\item
  Recommendations:
  ...
  \tightlist
  \item
    Recommendations for coordinating action across NYC and civil society
    going forward
  \end{itemize}
\end{itemize}
...
\item
  ...
  \item
    Synthesize and summarize relevant existing data and share in
    digestible format
  \item
...
\item
  ...
  \begin{itemize}
  \item
    Current data sources to guide City programs in emergencies as well
    as an assessment of their strengths and weaknesses
  \item
    Current data sources to guide civil society programs in emergencies
...
\item
  ...
  \tightlist
  \item
    Recommendations for effectively summarizing or generating, and using
    data to guide action in emergencies~
  \end{itemize}
\end{itemize}
...
\item
  ...
  \item
    Collect and disseminate community-made health communications
    materials
  \end{itemize}
\item
  Current Situation
  ...
  \begin{itemize}
  \item
    Current City practices for creating health communications materials
    in an emergency situation as well as an assessment of their
    strengths and weaknesses
  \item
    Current civil society practices for creating health communications
    materials in an emergency situation as well as an assessment of
...
\item
  ...
  \tightlist
  \item
    Recommendations for effectively creating and disseminating effective
    and culturally competent health communications materials in
    emergencies
  \end{itemize}
\end{itemize}
...
\item
  ...
  \begin{itemize}
  \item
    Current City practices for mobilizing and deploying resources in an
    emergency response as well as an assessment of their strengths and
    weaknesses
  \item
    Civil society practices for mobilizing and deploying resources in an
    emergency response as well as an assessment of their strengths and
    weaknesses
  \end{itemize}
\item
  ...
  \tightlist
  \item
    Recommendations for effectively mobilizing and deploying resources
...
This chapter will lay out a ``peace-time'' structure that monitors for
health emergencies and keeps stakeholders in the emergency response
connected, as well as a structure that can be rapidly built in order to
undertake the core functions outlined above. It will also include sample
budgets for different functions, and sample Terms of Reference documents
for the different roles outlined in the structure.~
...
And then provide TORs for each of the committees, as well as sample
budgets for coordination, surveillance, communications, and resourcing
activities.
...
This section outlines the different groups that will participate in the
process of developing the operational guidance, and shows key milestones
for the project.
...
To create legitimacy for the project among civil society members and
city officials, it should be led by a steering committee consisting of
civil society members who have convening power in their different
communities (e.g.~House Ball mothers, NGO directors, Party Promoters,
etc.), and city officials with institutional power. Civil society
members should receive honoraria. City members should be recognized by
the city for their contributions.

\hfill\break

Working Groups and Reviewers

The steering committee will then pull members of different communities
into working groups that will each work on one pillar of the operational
plan and invite colleagues to review the document once a draft is ready.
Civil society members should receive honoraria. City members should be
recognized by the city for their contributions.

\hfill\break

Observers

To cultivate an audience for the operational guidance, and to initiate a
national conversation about city-level emergency response, the steering
committee will invite peers (both City officials, and civil society
members) from other major U.S. cities. Observers will be invited to
steering committee and working group meetings and will be invited to
review the document.~
...
The project will be supported by consultants whose responsibilities will
include:

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  Conducting consultations, including working group meetings, steering
  committee meetings, and document reviews to write the
  ``Responsibilities under this function'', and ``Recommendations''
  sections.~
\item
  Interviewing key leaders and conducting document review to write
  ``Current Situation'' sections.
\item
  Creating video/audio of interviews and consultations for internal and
  external use.
\item
  Drafting the operational guidance.
\item
  Creating a website to host the operational guidance and related media.
\end{enumerate}
