...
participants 2000 times with replacement.
...
We say a separable exposure mapping \(g_{i}\) is symmetric with respect
to graph \(\mathscr{G}\) if for any
...
We say an exposure mapping \(g_i\) is invariant to its position in the
network if for \(i,j \in \mathscr{N}\) and \(t \in [1,k]\)
...
We say that an exposure mapping \(g_i\) is bounded if all possible
network exposure values are contained by a finite set \(\mathbf{D}\)
which does not depend on the size of the graph \(n\). i.e.~for
\(t \in [1,k]\):
...
Note that under classical causal inference \(g_t = c \in \mathbf{R}\)
for \(t \in [1,k]\). Classic causal inference models are trivially
loyal, separable, positionally invariant, symmetric, and bounded with
respect to the empty graph. Under network causal inference, boundedness
can obtain as a result of restrictions on the underlying network, on the
functional form of \(g_t\), or on the average of the entries of
\(\mathbf{X}_t\). The first situation would arise if node degree is
bounded, the second when we are modelling a network exposure which has a
ceiling effect in its dose-response curve, and the third when we are
considering a rare exposure.
