...
create_data_bipartite_graph_simulated <- function(bipartite_graph, config_list){
  
  n_reps = config_list[[1]][["settings"]][["n_reps_graph"]]
  ...
  old_edges            <- bipartite_graph |>
                                ...
                                tidygraph::mutate(from_name = tidygraph::.N()$name[from]) |>
                                tidygraph::mutate(to_name   = tidygraph::.N()$name[to]) |>
                                data.frame() |>
                                dplyr::mutate(
                                  to = to_name,
                                  old_from = from_name
                                  ) |>
                                {function(x) dplyr::select(x, old_from, to, names(x))}() |>
                                dplyr::select(-from) |>
                                dplyr::tibble()
  ...
  new_edges           <- data.frame(  
                                ...
                                {function(x) dplyr::select(x, from, to, rep, names(x))}()
