make_extended_example_network_data <- function(type = "bipartite", target_community = "Community a", vaccinated_communities = c()){
  ...
  example_edges <- data.frame(
    ...
    dplyr::mutate(col = ifelse(to == target_community, "#F73C95", "lightgrey")) 
  ...
  tidygraph::tbl_graph(nodes = node_data, edges = edge_data) |>
    dplyr::filter(!(name %in% vaccinated_communities))
