...
### MPX NYC Study Data Graph

Data were organized into the MPX NYC Study Graph for the purpose of analysis. The graph (or network) has nodes which represent participants and community districts, and edges, which connect the nodes. Each of these are represented as a table (i.e. there is a nodes table, and an edges table). We describe the tables below.

#### Nodes and Edges

##### Nodes Table

The nodes table represents each unique node in the MPX NYC Study Data graph as a row. There are two types of nodes (rows) in the study graph - participant nodes and community district nodes. Participant nodes (rows) each represent a person who took the MPX NYC survey. Community district nodes (rows) each represent one of the XX community districts of New York City. Participant characteristics such as age and gender identity are stored as columns in the nodes table. The first column of the table stores a unique identifyer for the node. These data comprise survey responses for person nodes and are missing for community district nodes.
...
The edges table represents each location a participant entered while completing their response as a row. The first and second columns of the table show the unique identifiers of the two nodes the edge links together. Locaton characterstics (such as location type) are stored as columns in this table. The MPX NYC Study Graph is a bipartite graph - the only allowable edges are those between a person and a community district.
...
The graph shows how participants affiliate with community districts in New York City. There are two related graphs that are more immediately interpretable, the MPX NYC Participant Graph, and the MPX NYC Community District Graph. These are each bipartite projections of the MPX NYC Study Data graph.

### MPX NYC Participant Graph

#### Nodes and Edges

The MPX NYC Participant Graph node table has survey participants as nodes. A weighted tie between two individuals indicates that they both have a residence or contact venue in the same community district. The node table is similar in structure as the big graph. The edge table shows the unique identifiers of the pair of nodes it connects, and a column for edge weights. The weights indicate the number of community districts the pair share. A tie of weight $0$ is equivalent to the absence of a tie.

#### Interpretation

The network encodes the potential for a pair of individuals to make contact with one another and transmit a pathogen. The more community districts they share, the more likely it is that they will make contact and/or transmit a pathogen. Of interest is the overall connectendess of the community overlap network. The better connected this network is, the more likely that a newly introduced infectious disease would disseminate quickly accross the population. Also of interest is the patterning of potential interactons between among participants from different demographic groups. It is useful to understand how different groups mix in space because it allows us to better predict the path of a newly introduced pathogen through different sections of the population.

##### Giant connected component

We calculated the proportion of participants contained by the largest connected component to understand how connected it is.

A connected component of a network is a subset of nodes where each node is reachable from any other node by tracing a path through network ties, and where there are no nodes excluded from the set which are reachable from a node included in the set. In many human systems, networks display a "giant connected component" - a connected component that contains the vast majority of the nodes in the network - with other, usually much smaller, connected components.

##### Spatial selection coefficient

We calculated the spatial selection coefficient - a measure of the propensity for members of one subgroup to have residences and contact venues in the same community districts of members of another, given subgroup.

The spatial selection coefficient is defined as the spatial selection ratio minus one. The spatial selection ratio is $1$ when contact venues and residences for the first subgroup are distributed in space independently of the other subgroup; greater than one if members of the former subgroup tend to share community districts with those of the latter; and smaller than one if the first sub-group tend to not share community districts with the second. The spatial selection coefficient is defined as the spatial selection ratio minus 1. We calculated the spatial selection coefficient across groups defined by race-gender, sexual orientation, and age.

More formally, consider a graph $G(N, \sim)$ where nodes (set $N$) represent participants and $i \sim j$ denotes that $i, j\in N$ are connected in graoh $G$. There is a tie between individuals $i,j \in N$ (i.e. $i \sim j$) if individuals $i$ and $j$ share a community district. We say individual $i$ shares a community district with individual $j$ if both have a residence or contact venue in that community district. $W_{i\sim j}$ is defined as the count of community districts shared by $i$ and $j$. $N_a, N_b \subset N$ are disjoint subsets of $N$ (i.e. $N_a \cap N_b = \phi$). By definition $W_{i \sim i} = 0, \forall i\in N$. We define $\psi(a,b)$ the spatial selection ratio of characteristic $a$ for characteristic $b$:

$$
\psi(a,b) = \frac{|N|}{|N_a||N_b|} \sum_{i \in N_a}{ \frac{  \sum_{j \in N_b}W_{i \sim j}}{  \sum_{j \in N}W_{i \sim j} } } 
$$

### MPX NYC Community District Graph

#### Nodes and Edges

The MPX NYC Community District Graph has community districts as nodes. Two community districts have a weighted tie between them if at least one individual has a residence or contact venue in both community districts (i.e. if they are both connected to the same community district in the MPX NYC person-place network). The weight of the tie is the count of such individuals. A tie of weight $0$ is equivalent to the absence of a tie.

#### Interpretation

This network encodes the potential for contagion to occur across community districts - the higher the number of individuals a pair of community districts shares, the higher the likelihood of transmitting a pathogen from one community district to the other. The fewer the participants that are common to two community districts, the less likely transmission is to occur between them.

Of interest is each community district's importance in the transmission dynamics of a novel pathogen, given the flow of people in and out of the district. We measured this using the spatial network centrality of each community district. Spatial network centrality is defined as weighted network degree in the participant overlap network.
...
#### MPX NYC person-place network

Data were collected and stored as the MPX NYC person-place network where nodes are survey participants as well as community districts. Ties can only exist between a participant and a community district - they indicate that the participant has a residence or contact venue in that community district. Tie characteristics encode the characteristics of contact venues.

Of interest is the pattern of movement from residences to contact venues. We calculated the distribytion of residences accross community districts, and separately the distribuytion of contact venues accross community districts. We calculated the difference in proportions for each community district, showing how survey participants move in the city.

This network yields two others that were used in the analysis - the spatial overlap network, and the participant overlap network described below.

#### Participant overlap network

The person overlap network has community districts as nodes. Two community districts have a weighted tie between them if at least one individual has a residence or contact venue in both community districts (i.e. if they are both connected to the same community district in the MPX NYC person-place network). The weight of the tie is the count of such individuals. A tie of weight $0$ is equivalent to the absence of a tie.

This network encodes the potential for contagion to occur across community districts - the higher the number of individuals a pair of community districts shares, the higher the likelihood of transmitting a pathogen from one community district to the other. The fewer the participants that are common to two community districts, the less likely transmission is to occur between them.

Of interest is each community district's importance in the transmission dynamics of a novel pathogen, given the flow of people in and out of the district. We measured this using the spatial network centrality of each community district. Spatial network centrality is defined as weighted network degree in the participant overlap network.

#### Community district overlap network

The community district overlap network has survey participants as nodes. A weighted tie between two individuals indicates that they both have a residence or contact venue in the same community district. The weights indicate the number of community districts the pair share. A tie of weight $0$ is equivalent to the absence of a tie.
