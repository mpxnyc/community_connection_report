...
It is possible to partition the node set $N$ into connected components. This follows from the facts that connected components form non-overalapping subsets of $N$ and that it is possible to write $N$ as a union of subsets which are each connected components. 

This partitioning is unique. Say there were two partitionings of $N$ into connected components $\{C_t\}$ and $\{C^*_t\}$. That would imply that 

$$ 
\exists i \in N \text{ such that } i \in C_l \text{, } i \in C^*_k \text{, and } C_l \not = C^*_k
$$
but that would imply that 

$$
\exists j \in C_l \text{ such that } j \not \in C^*_k 
$$

or
$$
\exists j \in C^*_k \text{ such that } j \not \in C_l 
$$

but since $i \in C_l$ and $i \in C^*_l$:

\begin{align}
j \in C_l &\Rightarrow \psi(i,j) > 0 \\

&\Rightarrow j \in C^*_k \\
...
\begin{align}
j \in C^*_k  &\Rightarrow \psi(i,j) > 0 \\
...
We can therefore uniquely express the node set $N$ as a union of connected components:

$$
N = \bigcup_{k = 1}^{K}{C_k}
$$

For a given graph $G$, we define the connected component size sequence $c_k \in \mathbb{N}$, $k \in [1, ..., K]$ as

$$
c_k = |C_k|
$$

Because connected components are mutually exclusive and exhaustive of the node set $N$, this sequence has the property that:
...
We define preference of group A for group B as the average proportion of alters from group B among egos from group A.

$$
\text{Pref}(A,B) = \frac{1}{|A|}\sum_{i \in A}{\frac{ R_i | B}{R_i}}
$$
...
$$
\text{Prev}(B) = \frac{|B|}{|N_p|}
$$

The spacial mixing coefficient is the ratio of preference to prevalence minus one:

$$
\phi(A,B) = \frac{\text{Pref(A,B)}}{\text{Prev}(B)} - 1
$$

#### Proportion in largest connected component

Given a graph $G$, we define the proportion in the largest connected component as:
