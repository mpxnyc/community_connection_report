...
It is possible to partition the node set $N$ into connected components. This follows from the facts that connected components form non-overalapping subsets of $N$ and that it is possible to write $N$ as a union of subsets which are each connected components. This partitioning is unique, we can therefore uniquely express the node set $N$ as a union of connected components:

$$
N = \bigcup_{k = 1}^{K}{C_k}
$$

#### Participant connected component size sequence
...
c_{p,k} = |C_{p,k}|
$$
Where $C_{p,k}$ is a sequency of connected components of $G_p(N_p, \psi_p)$
