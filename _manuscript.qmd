...
$$
G(N, \phi), \phi:N\times N \rightarrow \mathbb{Z}^+
$$ 

where $N$ is a set containing node indices and $\phi$ is a mapping from each possible pair of nodes to the set of positive integers. $\phi(a,b) = 0$ when there is no edge between nodes $a$ and $b$ and $\phi(a,b) > 0$ if there is.
...
$$
N = N_p \cup N_c \text{,   } N_p \cap N_c = \emptyset
$$
...
$$
\phi(a,b) > 0 \Rightarrow a\in N_p, b\in N_c
$$
...
$$
G_p(N_p, \phi_p), \phi_p:N_p\times N_p \rightarrow \mathbb{Z}^+
$$ 

where $N_p$ is a set containing node indices for participant nodes and $\phi_p$ is a mapping from each possible pair of participants to the set of positive integers. $\phi_p(a,b) = 0$ when there is no edge between participants $a$ and $b$ and $\phi_p(a,b) > 0$ if there is.

$G_p(N_p, \phi_p)$ can be derived from $G$ by taking the inner product of the $q \times r$ adjacency matrix $M$ with itself:

$$
M_p = MM^T
$$

The result is a $q \times q$ adjacency matrix $M_p$ whose entries $m_p^{ij}$ are used to define $\phi_p$:
...
*Edge weight*: the weight of an edge connecting two nodes is the count of community districts those two node share.

*Weighted node degree*: the weighted degree of each node is the sum of the edge weights for the edges connected to that node.

*Proportion in largest connected component*: a connected component of a network is a subset of nodes such that each node in the subset is reachable from any other node in the subset, and such that there are no nodes outside the subset which are reachable from a node inside it. By reachable, we mean possible to reach by tracing a path from one node to another over existing network ties. The proportion of nodes in the largest connected component is a measure of how well connected a graph is.
...
$$
G_c(N_c, \phi_c), \phi_c:N_c\times N_c \rightarrow \mathbb{Z}^+
$$ 

where $N_c$ is a set containing node indices for community district nodes and $\phi_c$ is a mapping from each possible pair of community districts to the set of positive integers. $\phi_c(a,b) = 0$ when there is no edge between community districts $a$ and $b$ and $\phi_c(a,b) > 0$ if there is.

$G_c(N_c, \phi_c)$ can be derived from $G$ by taking the inner product of the $r \times q$ matrix $M^T$ with itself:

$$
M_c = M^TM
$$

The result is a $r \times r$ matrix $M_c$, whose entries $m^{ij}_c$ are used to define $\phi_c$:
...
*Weighted degree*: the weighted degree of each node is the sum of the edge weights for the edges connected to that node.

## Intervention Algorithm

### Movement-neutralizing
Consider a sequence of graphs $G^{(k)}(N^{(k)}, \phi^{(k)})$ such that 
$$
G^{(0)}(N^{(0)}, \phi^{(0)}) = G(N, \phi)
$$
$G^{(k+1)}(N^{(k+1)}, \phi^{(k+1)})$ is defined as follows:
...
$$
n : max_{i \in N^{(k)}} \{Deg(G^{(k)},i)\}
$$
then

$$
\phi^{(k+1)}(a,b) = \phi^{(k)}(a,b), \forall a,b \in N^{(k+1)} 
$$
