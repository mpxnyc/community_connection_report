...
[Community district centrality]{.underline}

For each node in the community district network, we define community district centrality as the weighted degree of the node. (i.e. the sum of the weights for all the ties connected to the node).

### Home-venue movement network characteristics

We constructed the MPX NYC home-venue movement, which has community districts as nodes. There is a weighted directed edge between a pair of community districts if there is at least one participant whose residence is in the sending node (community district), and at least one contact venue is in the receiving node (community district).

[Net movement from home to contact venue community districts]{.underline}

We calculated net movement from home to contact venue community districts by subtracting in-degree from out-degree for each node and dividing by half of total-degree.
...
[Participant network]{.underline}

We calculated the spatial mixing coefficient across groups defined by age, sexual orientation, and race-gender.

[Home-venue movement network]{.underline}

We visualized net movement from home to contact venues using a bar graph. The overall pattern of movement from home to contact venues across community districts was visualzied by plotting the adjacency matrix for the home-venue movement network.
...
We define measures related to the egocentric network-related attributes of nodes in MPX NYC spatial participant network, the MPX NYC participant network, and the MPX NYC community district network. For each of these networks, node attributes have a slightly different interpretation. In addition, since there are two types of nodes in the MPX NYC spatial participant network, attributes have differing meaning depending on the type of node under consideration.

We defined four measures related to node degree which offer a useful interpretation of MPX NYC data: participant social reach ($R_i$), participant spacial reach ($S_i$), community district social reach ($r_j$), and community district spacial reach ($s_j$).

We further defined for each of these measures, a measure in which we count only the connections to nodes which are included in some defined subset $A \subset N$ of nodes. We thus define conditional participant social reach ($R_i|A$), conditional participant spacial reach ($S_i|A$), conditional community district social reach ($r_j|A$), and conditional community district spacial reach ($s_j|A$).
...
#### Participant spacial reach

Participant spacial reach is defined as the number of connections the focal participant has with community districts. It is defined as the weighted node degree in the MPX NYC spacial participant network. For $i \in N_p$
...
#### Community district spacial reach

Community district spacial reach is defined as the number of connections the focal community district has with other community districts through shared participants. For community districts $c$ and $d$, by "shared participants", we mean the set

$$ 
\{p : \psi(p,c) > 0 \text{ and } \psi(p, d) > 0 \, \text{ for } p \in N_p \}
$$ Community district spacial reach is defined as the weighted node degree in the MPX NYC community district network. For $j \in N_c$
...
The spacial mixing coefficient is the ratio of preference to prevalence minus one:
...
A connected component of $G(N, \psi)$ is a subset $C \subset N$ such that for $i,j \in C$, $i \not = j$ $$
\psi(i,j) > 0
$$
...
Singletons are trivially connected components.

It is possible to partition the node set $N$ into connected components. This follows from the facts that connected components form non-overalapping subsets of $N$ and that it is possible to write $N$ as a union of subsets which are each connected components. Since this partitioning is unique, we can uniquely express the node set $N$ as a union of connected components:

$$
N = \bigcup_{k = 1}^{K}{C_k}
$$

For the MPX NYC participant network $G_p$, we define the connected component size sequence $c_{p,k} \in \mathbb{N}$, $k \in [1, ..., K]$ with elements

$$
c_{p,k} = |C_{p,k}|
$$

Where $C_{p,k}$ is a sequence of connected components of $G_p(N_p, \psi_p)$

This sequence has the property that:

$$
|N_p| = \sum_{k = 1}^K{c_{p,k}}
$$

Given a graph $G_p$, we define the proportion in the largest connected component as:

$$
\frac{\max_k{c_{p, k}}}{|N_p|}
$$
...
The cumulative social network impact of immunizing $k$ community districts is measured in terms of the size of the largest connected component of the $G_p$ network as a proportion of the total number of participants

$$
\frac{\max_l{c^{(k)}_{p, l}}}{|N_p|}
$$
