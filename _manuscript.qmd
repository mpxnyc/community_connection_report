...
To describe the contact places elicited from participants, we cross-tabulated their characteristics.
To understand how participants move between home and contact places, for every community district, we subtracted the proportion of residences it contains from the proportion of contact places it contains. We further visualized the cross tabulation of home census tract and contact place census tract.
...
This network encodes the potential for a pair of individuals to make contact with one another and transmit a pathogen. The more community districts they share, the more likely it is that they will make contact and/or transmit a pathogen. Of interest is the overall connectedness of the network. The more connected a graph is, the more easily a pathogen could disseminate across the entire network.
...
### Spatial Mixing
The MPX NYC participant spatial overlap network can also be used to analyze the pattern of spatial overlap across different demographic groups. 

We calculated the spatial selection coefficient for age, sexual orientation, and race-gender. We define the spatial selection coefficient $\psi(a,b)$, a measure of the propensity for members of subgroup $a$ to have residences and contact venues in the same community districts as members of subgroup $b$. The spatial selection coefficient is defined as the spatial selection ratio minus one. i.e.
...
The second is the movement-based strategy, where community districts are ranked by their spatial network centrality. This strategy prioritizes community districts with a high level of connection to other community districts for immunization. 

To calculate spatial network centrality, we constructed a network with community districts as nodes. A weighted tie between two nodes indicates that the two community districts both have at least one individual who has a residence or community district in both places. The weight is given by the count of such individuals. For any given node in this network, we define spatial network centrality as the weighted degree of that node. (i.e. the sum of the weights for all the ties connected to the node).
