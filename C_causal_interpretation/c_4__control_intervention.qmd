...
Define $\phi(i, j)$ as an adjacency function - an indicator function that equals to $1$ if Person $i$ lives in or visits neighborhood $j$ and $0$ otherwise. Let $\mathscr{N}_P$ be the indicies of all person nodes and $\mathscr{N}_N$ be a set of indicies for neighborhood nodes. 


$$
\begin{align}
h_{sj}^1 &=  \sum_{ i \in \mathscr{N}_P} (1 -V_{(s-1), i}) \times \phi(i, j)       \\
h_{sj}^2 &=  \sum_{ i \in \mathscr{N}_P} (1 -V_{(s-1), i}) \times \phi(i, j) \sum_{ k \in \mathscr{N}_N} \phi(i, k) 
\end{align}
$$
...
Define graph $G^{s}(N, \phi_s)$ such that 

$$
\phi_s(i, j) = \phi(i, j) \times (1 -V_{(s-1), i})
$$

The cumulative social network impact of immunizing $s$ community districts is measured in terms of the size of the largest of the connected components $LCC_{(s)}$ of the $G_{s}(N, \phi_s)$ network as a proportion of the total number of participants.
