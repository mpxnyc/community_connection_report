...
@fig-homogenous-dag and @eq-homogenous-dag show the resulting non-parametric structural equation model for our data, if we analyse it under network causal inference. Because of the assumption of identical distributions, we can represent the model using three equations rather than 9. We retain the person subscript on the random variables in @eq-homogenous-dag and we keep all 9 nodes in the DAG on @fig-homogenous-dag and add six additional nodes to represent network exposure values with respect to $\mathbf{A}$ and $\mathbf{L}$. This is to clarify that though our model is for one person, it represents a part of a system that relates all the people in the sample with each other. Again, our original purpose was to infer something about causality in the context of a given population.
