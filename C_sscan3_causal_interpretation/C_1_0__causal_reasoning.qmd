...
It is possible to deduce conditional independence restrictions using the structural equations themselves. For instance, we can check whether two variables in a NPSEM are (conditionally) independent by checking whether both variables are a function of the same common error term $U_*$. It turns out that queries like this are much simpler to deduce through a graphical representation of the NPSEM than through the NPSEM itself. Directed acyclic graphs allow the transparent communication of conditional independence assumptions that define a given NPSEM. 
